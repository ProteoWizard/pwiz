...
STATUS_FILE: dict[str, str] = {
    ...
    "data/common/cvgen": "skipped",             # code-gen tool, ported as pwiz-sharp/build/CvGen
    ...
    "analysis/spectrum_processing/SpectrumList_IonMobility": "full",
...
DETAIL_FILE: dict[str, str] = {
    ...
    "data/common/cv": (
        ...
        "Ported as Common/CVID.generated.cs (~6.5K enum entries) plus CvLookup\n"
        "for term name/ID lookup. Generated from the OBO files by build/CvGen."
    ),
    ...
    "data/common/cvgen": "Code-gen tool - ported as build/CvGen (the enum half; the term tables are read from the OBOs at run time).",
    "data/common/cvgen_common": "Code-gen helpers - folded into build/CvGen/CvidEnumGenerator.cs.",
    ...
    "analysis/spectrum_processing/SpectrumList_IonMobility": "Unified IM/CCS wrapper across vendors + mzML re-read. Ported as Analysis/SpectrumList_IonMobility.cs. Dispatches through MsData/IIonMobilitySpectrumList (units + combined-IM + SONAR flag) and MsData/IIonMobilityCcsConversion (CCS dispatch), with an mzML CV-param sniff fallback. All six IM-capable vendor SpectrumLists implement IIonMobilitySpectrumList — Waters (drift time + SONAR), Bruker (1/K0, TIMS-only), Agilent (drift time), Mobilion (drift time), Thermo (compensation voltage), UIMF (drift time). CCS conversion is wired for Waters / Bruker / Mobilion / Agilent (the latter via MIDAC's ImsCcsInfoReader); Thermo and UIMF have no calibration source (matches cpp).",
