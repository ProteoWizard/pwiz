...
DETAIL_FILE: dict[str, str] = {
    ...
    "analysis/spectrum_processing/SpectrumList_IonMobility": "Unified IM/CCS wrapper across vendors + mzML re-read. Ported as Analysis/SpectrumList_IonMobility.cs. Dispatches through MsData/IIonMobilitySpectrumList (units + combined-IM + SONAR flag) and MsData/IIonMobilityCcsConversion (CCS dispatch), with an mzML CV-param sniff fallback. Waters implements both interfaces today; Agilent / Bruker / Mobilion / Thermo / UIMF plumb in via the same interfaces as their workflows land — the wrapper transparently uses the mzML sniff path until they do.",
