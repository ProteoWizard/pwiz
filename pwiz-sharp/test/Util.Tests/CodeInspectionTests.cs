...
/// <c>src/</c> + <c>test/</c>; hits aggregate by rule and surface together in one
...
public class CodeInspectionTests
{
    ...
    {
        // Walk src/ + test/ but skip build artifacts. EnumerateFiles is lazy and we bail on
        // any obj/bin segment so we don't read into nuget package caches.
        foreach (var top in new[] { "src", "test" })
        {
