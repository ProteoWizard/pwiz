...
call quickbuild.bat -q toolset=msvc --i-agree-to-the-vendor-licenses secure-scl=off %readerpath% > nul
if %ERRORLEVEL% EQU 0 GOTO SKIP
echo The mzML for Reader_%name% is out of date. Regenerating mzML...
call quickbuild.bat -q toolset=msvc --i-agree-to-the-vendor-licenses secure-scl=off --generate-mzML %readerpath% > nul
