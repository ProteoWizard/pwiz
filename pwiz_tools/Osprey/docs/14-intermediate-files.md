...
| classifier and tree settings (`--fdr-method gbdt` only) | a percolator directory re-run as gbdt, a gbdt directory written while the lean first pass still trained the SVM, or one `OSPREY_GBT_*` sweep point re-run as the next reports "skipping (outputs valid)" and hands back the other arm's results. Empty for `percolator`, so no existing directory is invalidated (`PercolatorEngine.GbdtValidityKeySuffix`) |
