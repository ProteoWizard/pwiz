...
| `--demux` | `off`, `auto` | `off` | Demultiplex overlapping-window (staggered) DIA to its narrow bins before searching, into `<stem>.demux.spectra.bin` (see [14-intermediate-files.md](14-intermediate-files.md)). `auto` leaves a non-overlapping run as acquired. With `off`, a run whose windows overlap is refused, since searching it as acquired scores every precursor in more than one window. Osprey-only. |
