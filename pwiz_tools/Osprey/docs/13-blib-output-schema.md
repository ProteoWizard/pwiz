...
- **`Osprey.Tasks/BlibOutputWriter.cs`** — the Stage 7 sequencer that composes rows from the passing-precursor lookup tables and drives `BlibWriter`. It is invoked once from `Osprey.Tasks/SecondPassFdrTask.cs:365` (the `SecondPassFDR` / `SecondPassFDR` HPC worker — see 15-hpc-scoring-split.md).
