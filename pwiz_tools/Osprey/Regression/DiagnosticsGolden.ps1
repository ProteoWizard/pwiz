...
function Test-DiagnosticsSanity {
    ...
    # The page must agree with itself about what it represents. Stated as an INTERNAL
    # consistency check rather than an expected value, so it holds for any report the gate
    # renders - the complete straight-through page and a first-pass-only worker page alike -
    # and needs no caller to say which it is looking at.
    #
    # The failure it exists to catch: a COMPLETE run whose page announces itself as
    # first-pass-only, or the reverse. Every number on such a page is correct, so no other
    # metric in either tier moves, and the reader is simply told the wrong thing about what
    # they are reading. Belongs here rather than in the golden projection because a golden
    # metric is regenerated by -CreateGolden, which would baptise exactly this regression.
    if ($null -eq $d.completeness) {
        $issues.Add("SANITY: the report carries no 'completeness' block, so it makes no statement about which passes it represents")
    } else {
        $hasPass2Content = $null -ne $d.pass2
        if ([bool]$d.completeness.pass2Present -ne $hasPass2Content) {
            $issues.Add(("SANITY: the report says pass2Present={0} but {1} pass-2 content; the page misdescribes what it represents" -f
                $d.completeness.pass2Present, $(if ($hasPass2Content) { 'carries' } else { 'carries no' })))
        }
        if ($d.completeness.runsExpected -ne $d.fileCount) {
            $issues.Add(("SANITY: completeness.runsExpected={0} disagrees with fileCount={1}" -f
                $d.completeness.runsExpected, $d.fileCount))
        }
        # A reason is required exactly when the page IS partial, and forbidden when it is not -
        # a banner on a complete report is as wrong as no banner on a partial one.
        $isPartial = -not [bool]$d.completeness.pass2Present -or
                     $d.completeness.runsContributed -lt $d.completeness.runsExpected
        $hasReason = -not [string]::IsNullOrWhiteSpace([string]$d.completeness.reason)
        if ($isPartial -ne $hasReason) {
            $issues.Add(("SANITY: the report is {0} but {1}" -f
                $(if ($isPartial) { 'partial' } else { 'complete' }),
                $(if ($hasReason) { 'states a reason for being partial' } else { 'states no reason' })))
        }
    }
