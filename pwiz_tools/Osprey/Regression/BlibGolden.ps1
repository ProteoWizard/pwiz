...
.SYNOPSIS
    ...
    comparison for the Osprey regression harness.

.DESCRIPTION
    Dot-source this file to get the blib-projection schema and the capture /
    compare functions used by the Osprey overnight regression
    (regression.ps1). NO dependency on the sibling ai/ checkout: the SQLite
    access uses the System.Data.SQLite.dll that ships in the Osprey build
    ...
    comparator (ai/scripts/Osprey/Compare/Compare-Blib-Crossimpl.ps1) but
...
# ----------------------------------------------------------------------
# SQLite access (System.Data.SQLite from the Osprey build output)
# ----------------------------------------------------------------------
