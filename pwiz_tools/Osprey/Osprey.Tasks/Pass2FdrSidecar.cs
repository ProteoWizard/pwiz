...
namespace pwiz.Osprey.Tasks
{
    ...
    internal static class Pass2FdrSidecar
    {
        ...
        /// </summary>
        /// <summary>
        /// Reduce the experiment-q floors from each run's ARTIFACTS rather than from its
        /// materialized survivor pool, returning false when any run cannot supply them (the caller
        /// then folds the pool as before).
        ///
        /// <para>The floors are two minima over one number per row - <c>max(run precursor q, run
        /// peptide q)</c> - keyed by entry id and by (modified sequence, decoy). Every one of those
        /// numbers is already in the run's 2nd-pass FDR sidecar, keyed by entry id, and the
        /// sidecar is that run's POOL IMAGE: one record per reconciled row, written in the same
        /// canonical order. Folding the pool instead meant, per run, materializing ~4 M
        /// <see cref="FdrEntry"/> objects from the reconciled parquet, a whole-file
        /// <c>Dictionary&lt;uint,int&gt;</c> to overlay the 1st-pass sidecar onto them, and a
        /// whole-file <c>Dictionary&lt;uint,FdrEntry&gt;</c> to overlay the 2nd-pass sidecar - all
        /// to read four fields off each row and drop the rest. On a 446-run cohort that pass was
        /// the tallest committed-memory excursion in the process (issue #4664).</para>
        ///
        /// <para>The parquet is still read, for the two columns the sidecar does not carry -
        /// <c>modified_sequence</c> and <c>is_decoy</c> - through
        /// <see cref="ParquetScoreCache.StreamRowIdentity"/>, which builds no entries and interns
        /// sequences exactly as the pool load does. The two sides are joined in LOCKSTEP on entry
        /// id, which is what the equal counts and shared canonical order buy; a row that does not
        /// line up is an artifact disagreement and throws rather than quietly folding the wrong
        /// number into a boundary.</para>
        ///
        /// <para>All-or-nothing by design: every run's artifacts are checked before any run is
        /// folded, so the caller never sees a half-folded pair of dictionaries. Even that is
        /// belt-and-braces - both paths reduce the same values by MIN, so folding a run twice
        /// cannot change the answer.</para>
        /// </summary>
        internal static bool TryFoldExperimentQFloorsFromArtifacts(
            PipelineContext ctx, RescoredEntries rescored,
            Dictionary<uint, double> minRunBothByEntryId,
            Dictionary<(string ModifiedSequence, bool IsDecoy), double> minRunBothByPeptide)
        {
            // Every decline says WHY. A fast path that silently falls back is one nobody notices
            // has stopped running: the answer stays right and the cost quietly returns.
            if (!rescored.Streams)
            {
                ctx.LogInfo(@"Experiment-q floors: the survivor pool is resident, so the floors are folded from it directly.");
                return false;
            }
            var parquetPaths = ctx.Get<PerFileParquetPaths>().Value;
            if (parquetPaths == null)
            {
                ctx.LogInfo(@"Experiment-q floors: no per-file parquet paths, so the floors are folded from the survivor pools.");
                return false;
            }
            var writer = new Pass2SidecarWriter(ctx, ctx.Config, null, null);

            // Phase 1: every run, or none. A run with no current 2nd-pass sidecar is the case the
            // streamed overlay logs and carries 1st-pass q-values for, and this fold has no way to
            // reproduce that - so it declines the whole cohort rather than mixing two rules.
            var byFile = new List<(string FileName, string ReconciledPath, string SidecarPath)>();
            foreach (string fileName in rescored.FileNames)
            {
                string why = null;
                string reconciledPath = null;
                string sidecarPath = null;
                if (!parquetPaths.TryGetValue(fileName, out string scoresPath))
                {
                    why = @"it has no scores parquet path";
                }
                else if (!File.Exists(reconciledPath = ParquetScoreCache.ReconciledPathFromScoresPath(scoresPath)))
                {
                    why = string.Format(@"its reconciled parquet is not on disk ({0})", reconciledPath);
                }
                else
                {
                    string inputFile = writer.InputFor(fileName);
                    if (inputFile == null)
                    {
                        why = @"it matches no configured input file";
                    }
                    else if (!FdrScoresSidecar.IsCurrentFormat(
                                 sidecarPath = FdrScoresSidecar.Pass2Path(inputFile),
                                 FdrScoresSidecar.Pass.SecondPass))
                    {
                        why = string.Format(@"it has no current 2nd-pass FDR sidecar ({0})", sidecarPath);
                    }
                }
                if (why != null)
                {
                    ctx.LogInfo(string.Format(
                        @"Experiment-q floors: folding from the survivor pools instead, because {0} {1}.",
                        fileName, why));
                    return false;
                }
                byFile.Add((fileName, reconciledPath, sidecarPath));
            }
            if (byFile.Count == 0)
            {
                ctx.LogInfo(@"Experiment-q floors: no runs to fold.");
                return false;
            }

            // Phase 2: fold. The record buffer and the interner are the cohort's, not the file's.
            var records = new List<FdrScoreRecord>();
            var sequencePool = new LibraryStringInterner();
            using (var progress = new ProgressReporter(
                       string.Format(@"Folding experiment-q floors from {0} run(s) of sidecars", byFile.Count),
                       byFile.Count, string.Empty, ProgressReporter.IO_INTERVAL_SECONDS))
            {
                int done = 0;
                foreach (var file in byFile)
                {
                    ...
                    records.Clear();
                    if (!FdrScoresSidecar.ReadRecords(file.SidecarPath, FdrScoresSidecar.Pass.SecondPass,
                            rec => records.Add(rec)))
                    {
                        ctx.LogWarning(string.Format(
                            @"Experiment-q floors: {0} failed to read, so the floors are folded from the survivor pools instead.",
                            file.SidecarPath));
                        return false;
                    }
                    int row = 0;
                    ParquetScoreCache.StreamRowIdentity(file.ReconciledPath, sequencePool,
                        (entryId, isDecoy, modSeq) =>
                        {
                            if (row >= records.Count || records[row].EntryId != entryId)
                            {
                                throw new InvalidDataException(string.Format(
                                    @"Experiment-q floors: {0} row {1} is entry {2} but its 2nd-pass sidecar has {3} there. The sidecar is supposed to be this run's pool image, one record per reconciled row in the same order.",
                                    file.FileName, row, entryId,
                                    row < records.Count ? records[row].EntryId.ToString() : @"no record"));
                            }
                            var rec = records[row++];
                            PercolatorEngine.AccumulateExperimentQFloorRow(entryId, isDecoy, modSeq,
                                Math.Max(rec.RunPrecursorQvalue, rec.RunPeptideQvalue),
                                minRunBothByEntryId, minRunBothByPeptide);
                        });
                    if (row != records.Count)
                    {
                        throw new InvalidDataException(string.Format(
                            @"Experiment-q floors: {0} has {1} reconciled row(s) against {2} record(s) in its 2nd-pass sidecar.",
                            file.FileName, row, records.Count));
                    }
                }
            }
            ctx.LogInfo(string.Format(
                @"Experiment-q floors: folded {0} run(s) from their 2nd-pass sidecars and reconciled identity columns; no survivor pool was materialized.",
                byFile.Count));
            return true;
        }
