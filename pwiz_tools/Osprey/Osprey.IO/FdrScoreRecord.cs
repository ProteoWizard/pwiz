...
namespace pwiz.Osprey.IO
{
    ...
    /// PEP + run_protein_qvalue), decoupled from any resident buffer (issue #4355
    /// struct-shrink S0). Since the lean <c>FdrProjection</c> no longer carries the
    /// q-value outputs, the projection sidecar writers assemble records of this shape
    /// -- the 1st pass from the lean row's <c>EntryId</c>/<c>Score</c> plus the parallel
    ...
    /// The 60-byte byte layout stays single-sourced through
