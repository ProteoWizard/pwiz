...
namespace pwiz.Osprey.Test
{
    ...
    public class OspreyFeatureCalculatorsTest
    {
        ...
        /// <summary>
        /// --extra-features must be additive and OFF by default: the 21 PIN indices keep
        /// their identity and the vector stays 21 wide unless asked for. This is the
        /// invariant that keeps the flag byte-neutral for Rust cross-impl parity and the
        /// regression golden -- if the extras were ever inserted rather than appended,
        /// every PIN index downstream would silently shift.
        /// </summary>
        [TestMethod]
        public void TestExtraFeaturesAreAppendedNotInserted()
        {
            Assert.AreEqual(21, OspreyFeatureCalculators.FeatureCount);
            Assert.AreEqual(21, OspreyFeatureCalculators.Count(false));
            Assert.AreEqual(21 + OspreyFeatureCalculators.ExtraFeatureCount,
                OspreyFeatureCalculators.Count(true));

            // The parquet column names and the calculators must agree at every index --
            // they live in different assemblies (Osprey.IO cannot see Osprey.Scoring) and
            // are paired only by position, so a mismatch would silently mislabel columns.
            string[] baseNames = ParquetScoreCache.FeatureNames(false);
            string[] wideNames = ParquetScoreCache.FeatureNames(true);
            Assert.AreEqual(OspreyFeatureCalculators.Count(false), baseNames.Length);
            Assert.AreEqual(OspreyFeatureCalculators.Count(true), wideNames.Length);
            for (int i = 0; i < wideNames.Length; i++)
            {
                Assert.AreEqual(wideNames[i], OspreyFeatureCalculators.Get(i).Name,
                    string.Format("feature {0}: parquet column name and calculator disagree", i));
                if (i < baseNames.Length)
                    Assert.AreEqual(baseNames[i], wideNames[i],
                        string.Format("feature {0}: PIN name changed when the extras were added", i));
            }
        }

        /// <summary>
        /// peak_symmetry: left-of-apex trapezoidal area over right-of-apex, capped at 10,
        /// and 1.0 for a degenerate peak. The U shape about 1.0 -- fronting &lt; 1 &lt;
        /// tailing, both bad -- is exactly why it is tree-only.
        /// Mirrors Rust pipeline.rs:7442.
        /// </summary>
        [TestMethod]
        public void TestPeakSymmetryCalculator()
        {
            var rts = new double[] { 0, 1, 2, 3, 4, 5, 6 };
            // Reference XIC (higher total): rises fast to the apex, then tails off --
            // a FRONTING peak, so left area < right area and symmetry < 1.
            var frag0 = new double[] { 0, 10, 100, 40, 20, 10, 0 };   // total 180
            var frag1 = new double[] { 0, 5, 20, 10, 5, 2, 0 };       // total 42
            var xics = new List<XicData>
            {
                new XicData(0, rts, frag0),
                new XicData(1, rts, frag1),
            };
            var bounds = new XICPeakBounds { StartIndex = 1, EndIndex = 5, ApexIndex = 2 };
            var context = new OspreyScoringContext(null);
            context.ClearByproducts();

            double symmetry = OspreyFeatureCalculators.Get(22).Calculate(
                context, new FakePeakData(xics, bounds));

            // left  = trapezoid over [1,2]: (10+100)/2 * 1                    = 55
            // right = trapezoid over [2,5]: 70 + 30 + 15                      = 115
            Assert.AreEqual(55.0 / 115.0, symmetry, TOLERANCE);
            Assert.IsTrue(symmetry < 1.0, "a fronting peak must report < 1.0");
        }

        /// <summary>
        /// hyperscore's ln(n!) terms go through <see cref="ScoringMath.LogGamma"/>
        /// (ln(Gamma(n+1)) == ln(n!)). Verify it against directly-computed factorials, so
        /// the hyperscore rests on a checked implementation, not only on matching the Rust
        /// source by inspection. Also pins the short-circuits (n &lt;= 2 -> 0; x &lt;= 0 -> 0).
        /// </summary>
        [TestMethod]
        public void TestLogGammaMatchesFactorials()
        {
            // ln(Gamma(1)) = ln(0!) = 0; ln(Gamma(2)) = ln(1!) = 0.
            Assert.AreEqual(0.0, ScoringMath.LogGamma(1.0), 1e-9);
            Assert.AreEqual(0.0, ScoringMath.LogGamma(2.0), 1e-9);

            // ln(Gamma(k+1)) == ln(k!) for k = 2..12, against an exact running factorial.
            double factorial = 1.0;   // 1! at k = 1
            for (int k = 2; k <= 12; k++)
            {
                factorial *= k;                       // now k!
                double expected = Math.Log(factorial); // ln(k!)
                double actual = ScoringMath.LogGamma(k + 1.0);
                Assert.AreEqual(expected, actual, 1e-6,
                    string.Format("LogGamma({0}) should equal ln({1}!) = {2}", k + 1, k, expected));
            }

            // Domain guards: non-positive returns 0 (not a throw / NaN).
            Assert.AreEqual(0.0, ScoringMath.LogGamma(0.0), 1e-12);
            Assert.AreEqual(0.0, ScoringMath.LogGamma(-3.0), 1e-12);
        }

        /// <summary>
        /// fragment_coelution_min: the WEAKEST pairwise fragment correlation. Must differ
        /// from the max the PIN already carries -- that separation ("one good pair vs all
        /// pairs good") is the whole point of the feature.
        /// </summary>
        [TestMethod]
        public void TestCoelutionMinCalculator()
        {
            var rts = new double[] { 0, 1, 2, 3, 4, 5 };
            // frag0 and frag1 co-elute tightly; frag2 is anti-correlated with both, so the
            // MIN is strongly negative while the MAX stays high.
            var xics = new List<XicData>
            {
                new XicData(0, rts, new double[] { 1, 10, 50, 90, 40, 5 }),
                new XicData(1, rts, new double[] { 2, 12, 55, 88, 38, 6 }),
                new XicData(2, rts, new double[] { 90, 60, 30, 5, 20, 70 }),
            };
            var bounds = new XICPeakBounds { StartIndex = 0, EndIndex = 5, ApexIndex = 3 };
            var context = new OspreyScoringContext(null);
            context.ClearByproducts();
            var peakData = new FakePeakData(xics, bounds);

            double max = OspreyFeatureCalculators.Get(1).Calculate(context, peakData);
            double min = OspreyFeatureCalculators.Get(21).Calculate(context, peakData);

            Assert.IsTrue(max > 0.9, string.Format("expected a tightly co-eluting pair, got max={0}", max));
            Assert.IsTrue(min < 0.0, string.Format("expected an anti-correlated pair, got min={0}", min));
            Assert.IsTrue(min < max, "min must not collapse onto max");
        }
