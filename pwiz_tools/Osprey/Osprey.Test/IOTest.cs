...
<mzML xmlns=""http://psi.hupo.org/ms/mzml"">
  <run>
    <spectrumList count=""1"" defaultDataProcessingRef=""dp"">"
        ...
        /// <summary>
        /// --extra-features widens the feature vector to 25 (21 PIN + 4 extra); the parquet
        /// must round-trip that width. This pins the whole width-plumbing chain: the writer
        /// infers 25 from the vectors and names the extra columns, and the reader detects 25
        /// from the file's own schema (NOT a flag) and reads them back. A regression here
        /// would silently drop or mislabel the extra scores -- the failure mode the
        /// data-derived width was built to prevent.
        /// </summary>
        [TestMethod]
        public void TestExtraFeaturesParquetRoundTrip()
        {
            int wide = ParquetScoreCache.NUM_PIN_FEATURES + ParquetScoreCache.EXTRA_FEATURE_NAMES.Length;
            Assert.AreEqual(25, wide, "21 PIN + 4 extra");

            var entries = new List<FdrEntry>();
            for (int i = 0; i < 3; i++)
            {
                var feats = new double[wide];
                for (int f = 0; f < wide; f++)
                    feats[f] = f + i * 0.5;      // distinct per (row, column) so a mis-map surfaces
                entries.Add(new FdrEntry
                {
                    EntryId = (uint)(i + 1),
                    IsDecoy = false,
                    ModifiedSequence = "PEPTIDE" + i,
                    Charge = 2,
                    ScanNumber = (uint)(100 + i),
                    ApexRt = 4.5,
                    StartRt = 4.0,
                    EndRt = 5.0,
                    Features = feats,
            ...
            string path = Path.Combine(Path.GetTempPath(),
                "osprey_test_wide_" + Guid.NewGuid().ToString("N") + ".parquet");
            try
            {
                ParquetScoreCache.WriteScoresParquet(path, entries, null, null, "wide.mzML");

                // Reader detects width from the file schema; every row comes back 25-wide.
                var rows = ParquetScoreCache.LoadPinFeaturesFromParquet(path);
                Assert.AreEqual(3, rows.Count);
                foreach (var r in rows)
                    Assert.AreEqual(wide, r.Length, "each vector round-trips at the widened width");

                // Values survive, including columns 21..24 (the extras). If the writer had
                // dropped them or the reader ignored them, the width would have come back
                // 21 and the assert above would already have failed -- so a correct 25-wide
                // read with correct tail values IS proof the extra columns are present and
                // named. Rows are re-sorted by (entry_id, charge, scan) = insertion order
                // here, so row r corresponds to entries[r].
                for (int i = 0; i < 3; i++)
                    for (int f = 0; f < wide; f++)
                        Assert.AreEqual(f + i * 0.5, rows[i][f], 1e-9,
                            string.Format("row {0} col {1} value must survive the round-trip", i, f));

                // And a legacy 21-wide write still reads back 21 (the flag-off path is
                // unchanged): confirmed via FeatureNames, the single source both sides key off.
                Assert.AreEqual(21, ParquetScoreCache.FeatureNames(false).Length);
                Assert.AreEqual(25, ParquetScoreCache.FeatureNames(true).Length);
            }
            finally
            {
