...
namespace TestPerf // Note: tests in the "TestPerf" namespace only run when the global RunPerfTests flag is set
{
    ...
    public class MeasuredDriftTimesPerfTest : AbstractFunctionalTest
    {
        ...
        protected override void DoTest()
        {
            ...
            // And finally verify ability to reimport with altered drift filter (would fomerly fail on an erroneous Assume)
            ...
            RunUI(() =>
            {
                ...
                foreach (var entry in oldPredictor.MeasuredDriftTimePeptides)
                {
                    dict.Add(entry.Key, entry.Value.ChangeDriftTimeMsec(entry.Value.DriftTimeMsec??0 + .5));
                }
                driftTimePredictorDlg2.Predictor = oldPredictor.ChangeMeasuredDriftTimePeptides(dict).ChangeName("test") as DriftTimePredictor;
