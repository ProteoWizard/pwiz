...
namespace TestPerf
{
    ...
    public class CarafeBuildLibraryTutorial : AbstractFunctionalTestEx
    {
        ...
        private string ProteinDatabase => TestFilesDir.GetTestPath(@"UP000005640_9606_small.fasta");
        string DiannFineTuneFile => TestFilesDir.GetTestPath(@"report.tsv");


        private void TestCarafeBuildLibrary()
        {
            ...
            Directory.CreateDirectory(Path.GetDirectoryName(LibraryPathTunedByDiann) ?? string.Empty);
            // We're on the "Name" page of the wizard.
            RunUI(() =>
            {
                buildLibraryDlg.LibraryName = LibraryTunedByDiann;
                buildLibraryDlg.LibraryPath = LibraryPathTunedByDiann;
            ...
            RunUI(() =>
            {
                ...
                buildLibraryDlg.SelectedCarafeOptions = BuildLibraryDlg.CarafeOptions.tabCarafeData;
            });
            PauseForScreenShot<BuildLibraryDlg>("Build Library - Carafe Settings, Training Data Generation");
            RunUI(() => { buildLibraryDlg.SelectedCarafeOptions = BuildLibraryDlg.CarafeOptions.tabCarafeModel; });
            PauseForScreenShot<BuildLibraryDlg>("Build Library - Carafe Settings, Model Training");
            RunUI(() => { buildLibraryDlg.SelectedCarafeOptions = BuildLibraryDlg.CarafeOptions.tabCarafeLibrary; });
            PauseForScreenShot<BuildLibraryDlg>("Build Library - Carafe Settings, Library Generation");

            RunUI(() =>
            {
                buildLibraryDlg.OkWizardPage();
                buildLibraryDlg.TextBoxMsMsDataFile = MzMLFile;
                buildLibraryDlg.ComboBuildLibraryTarget =
                    BuildLibraryDlg.BuildLibraryTargetOptions.currentSkylineDocument;
                //buildLibraryDlg.TextBoxProteinDatabase = ProteinDatabase;
                buildLibraryDlg.ComboLearnFrom = BuildLibraryDlg.LearningOptions.diann_report;
                buildLibraryDlg.TextBoxTrainingDoc = DiannFineTuneFile;


            });

            PauseForScreenShot<BuildLibraryDlg>("Build Library - Carafe Tuning Files set");
            ...
            var builtLibraryPath = carafeLibraryBuilder.CarafeOutputLibraryFilePath;
            ...
            RunUI(() =>
            {
                peptideSettingsUI.PickedLibraries = new [] { LibraryTunedByDiann };
                peptideSettingsUI.SetSelectedLibrary(LibraryTunedByDiann);
            });
