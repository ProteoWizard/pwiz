...
namespace TestPerf // Note: tests in the "TestPerf" namespace only run when the global RunPerfTests flag is set
{
    ...
    public class MeasuredInverseK0PerfTest : AbstractFunctionalTestEx
    {
        ...
        protected override void DoTest()
        {
            ...
            PauseForManualTutorialStep("Bruker TIMS data, reimported with IM filtering. Observed IM is the per-channel intensity-weighted mean (center of gravity) of the mobilogram; cross-isotope abundance weighting is applied at the group level. Open the full-scan view on a peak point (right-click chromatogram > 'Show full scan'). Things to verify: (1) BOTH the heatmap purple band AND the IM-vs-intensity sidecar should show two dark-violet lines - DASHED at target IM, DOTTED at the selected transition's per-PEAK observed IM, stable across scans within the peak (the Document Grid's per-precursor Observed Ion Mobility is the abundance-weighted aggregate of these). (2) Hovering near either line shows a tooltip - target line: Target [units] / Target CCS; observed line: Measured [units] / IM error % / Measured CCS / CCS error %. (3) Hover ON a data point - the data point tooltip wins (lines are lower priority).");
