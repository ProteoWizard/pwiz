...
namespace TestPerf // Note: tests in the "TestPerf" namespace only run when the global RunPerfTests flag is set
{
    ...
    /// </summary>
    [TestClass]
    public class TestCommandlineCreateImsDbPerf : AbstractFunctionalTestEx
    {
        ...
        protected override void DoTest()
        {
            ...
            var outSky = TestFilesDirs[0].GetTestPath("ImsDbTest.sky");
            var transitionList = TestFilesDirs[0].GetTestPath("test_run_1_transition_list.csv");
            var imsdb = TestFilesDirs[0].GetTestPath("ImsDbTest.imsdb");
            var outCSV = TestFilesDirs[0].GetTestPath("ImsDbTest.csv");
            var output = RunCommand($"--in={template}",
                $"--out={outSky}",
                $"--import-transition-list={transitionList}",
                $"--import-file={inFile}",
                $"--ionmobility-library-create={imsdb}",
                "--ionmobility-library-name=ImsDbTest",
