    ...
    public partial class ImportTransitionListColumnSelectDlg : ModeUIInvariantFormEx
    {
        ...
        private void DisplayData()
        {
            ...
            else
            {
                for (var i = 0; i < numColumns; i++)
                {
                    // In the case when we don't have user provided headers, we still want localized headers that can be translated,
                    // this replaces the autogenerated strings with a localized version
                    dataGrid.Columns[i].HeaderText = string.Format(Resources.ImportTransitionListColumnSelectDlg_DisplayData_Column_) + (i+1).ToString();
