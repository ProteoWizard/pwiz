...
{
    /// <summary>
    /// Exporting a report or annotations starts in the folder the DOCUMENT is saved in, and remembers nothing:
    /// sending one export somewhere else does not make that folder the default for every export after it
    /// (https://skyline.ms/announcements/home/support/thread.view?rowId=72321). The folder Skyline does
    /// remember, Settings.Default.ExportDirectory, belongs to method export alone - these dialogs must neither
    /// read it nor write it.
    /// </summary>
    [TestClass]
    public class ExportDirectoryTest : AbstractFunctionalTest
    {
        private const string DOCUMENT_NAME = @"ExportDirectoryTest";
        ...
        {
            _documentDir = TestContext.GetTestResultsPath(@"Document");
            _otherDir = TestContext.GetTestResultsPath(@"Other");
            _methodDir = TestContext.GetTestResultsPath(@"Method");
        ...
        /// folder. Left unset instead, the dialog would open whereever a Skyline file dialog was last used,
