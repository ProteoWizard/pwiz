...
namespace pwiz.SkylineTestFunctional
{
    ...
    public class HighReplicateCountTest : AbstractFunctionalTestEx
    {
        ...
        protected override void DoTest()
        {
            ...
            Skyline.SkylineWindow.MAX_GRAPH_CHROM = 10; // Normally 100, reduce for quicker test
            int TOO_MANY_FILES = Skyline.SkylineWindow.MAX_GRAPH_CHROM * 2; 
            int count;
            for (count = 0; count < TOO_MANY_FILES; count++) 
            {
                var fname = TestFilesDir.GetTestPath((count==Skyline.SkylineWindow.MAX_GRAPH_CHROM ? "distinct" : "R" + count) + ExtensionTestContext.ExtMzml);
                filenames.Add(fname);
