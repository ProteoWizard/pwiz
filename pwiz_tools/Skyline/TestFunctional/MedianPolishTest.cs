...
namespace pwiz.SkylineTestFunctional
{
    ...
    {
        [TestMethod]
        public void TestMedianPolishProteinAbundance()
        {
            TestFilesZip = @"TestFunctional\MedianPolishTest.zip";
            RunFunctionalTest();
        }
        ...
        protected override void DoTest()
        {
            ...
            RunDlg<ExportLiveReportDlg>(SkylineWindow.ShowExportReportDialog, exportLiveReportDlg =>
            {
                ...
                exportLiveReportDlg.SetUseInvariantLanguage(true);
                exportLiveReportDlg.OkDialog(
                    TestFilesDir.GetTestPath("MedianPolishedAreas-Plasma-MagNet-Elute-Small.parquet"));
            });

            var document = SkylineWindow.Document;
            var expectedMedianPolishedAreas = ReadExpectedMedianPolishedAreas(document, TestFilesDir.GetTestPath("ExpectedMedianPolishedAreas.parquet"));
            ...
            foreach (var moleculeGroup in document.MoleculeGroups)
            {
                foreach (var molecule in moleculeGroup.Molecules)
                {
                    ...
                    CollectionAssert.AreEqual(expected, medianPolishedValue, $"Median polished values do not match for peptide {molecule.ModifiedSequence}.");
                }
            }
        }
