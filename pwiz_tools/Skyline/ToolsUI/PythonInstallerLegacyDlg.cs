...
namespace pwiz.Skyline.ToolsUI
{
    public partial class PythonInstallerLegacyDlg : FormEx
    {
        ...
        public PythonInstallerLegacyDlg(ProgramPathContainer pythonPathContainer, IEnumerable<string> packages, TextWriter writer)
            : this(pythonPathContainer, packages, PythonUtil.CheckInstalled(pythonPathContainer.ProgramVersion), writer)
        {
        }

        public PythonInstallerLegacyDlg(ProgramPathContainer pythonPathContainer, IEnumerable<string> packages, bool installed, TextWriter writer)
        {
        ...
        private bool GetPackages()
        {
            ...
            try
            {
                ...
                if (exePaths.Count != 0)
                {
                    using (var waitDlg = new LongWaitDlg(null, false))
                    {
                        waitDlg.Message = ToolsResources.PythonInstaller_GetPackages_Installing_Packages;
