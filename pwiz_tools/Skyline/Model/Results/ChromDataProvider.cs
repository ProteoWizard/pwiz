    ...
    internal sealed class SpectraChromDataProvider : ChromDataProvider
    {
            ...
            : base(status, startPercent, endPercent, loader)
        {
            ...
            for (int i = 0; i < lenSpectra; i++)
            {
                ...
                {
                    // TODO: Implement correct support for high accuracy full scans filtering
                    if (filter.IsHighAccProductFilter)
                        throw new InvalidDataException("Full-scan MS/MS filtering of high accuracy spectra is not yet supported.  Send us your data.");

                    // Centroiding for high accuracy data did not work that well
//                    var dataSpectrum = (filter.IsHighAccProductFilter
//                                        ? dataFile.GetCentroidedSpectrum(i)
//                                        : dataFile.GetSpectrum(i));

                    var dataSpectrum = dataFile.GetSpectrum(i);

                    double? rt = dataSpectrum.RetentionTime;
                    if (rt.HasValue && dataSpectrum.Level == 2)
                    {
                        ...
                        foreach (var spectrum in filter.SrmSpectraFromFullScan(rt,
                            dataSpectrum.PrecursorMz, dataSpectrum.Mzs, dataSpectrum.Intensities))
                        {
    ...
    internal sealed class SpectrumFilter
    {
        ...
        public SpectrumFilter(SrmDocument document)
        {
            ...
            if (Enabled)
            {
                ...
                if (instrument.ProductFilter.HasValue)      // Keep resharper happy
                    _productFilterWindow = instrument.ProductFilter.Value;
                if (_isHighAccProductFilter)
                    _productFilterWindow = _productFilterWindow/MILLION; // ppm
            ...
            double[] mzArray, double[] intensityArray)
        {
            ...
            foreach (var filterPair in FindFilterPairs(precursorMz.Value, _precursorFilterType))
            {
                ...
                for (int i = 0; i < mzArrayNew.Length; i++)
                {
                    ...
                    double mzQ3 = mzArrayNew[i];
                    double filterWindow = _isHighAccProductFilter
                                              ? mzQ3*_productFilterWindow
                                              : _productFilterWindow;

                    double startFilter = mzArrayNew[i] - filterWindow / 2;
