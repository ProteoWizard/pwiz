...
//                }
    ...
    internal sealed class SpectraChromDataProvider : ChromDataProvider
    {
            ...
            : base(status, startPercent, endPercent, loader)
        {
            ...
            var chromMap = new ChromDataCollectorSet(dataFile.HasSrmSpectra
                ? ChromDataCollectorSet.ScanType.srm
                : ChromDataCollectorSet.ScanType.ms2);
            var chromMapMs1 = new ChromDataCollectorSet(ChromDataCollectorSet.ScanType.ms1);
            ...
            for (int i = 0; i < lenSpectra; i++)
            {
                ...
                else if (filter.EnabledMsMs || filter.EnabledMs)
                {
                    ...
                    if (dataSpectrum.Level == 1 && filter.EnabledMs)
                    {
                        ...
                        int? insertIndex = AddTime((float) rt.Value, chromMapMs1.Times);
    ...
    {
        public enum ScanType { srm, ms1, ms2 }

        public ChromDataCollectorSet(ScanType scanType)
        {
            TypeOfScans = scanType;
            PrecursorCollectorMap = new Dictionary<double, ChromDataCollector>();
            if (scanType == ScanType.ms1)
                Times = new List<float>();
        }

        public ScanType TypeOfScans { get; private set; }

        public bool IsSrm { get { return TypeOfScans == ScanType.srm; } }
        public bool IsMs2 { get { return TypeOfScans == ScanType.ms2; } }
    ...
    internal sealed class ChromDataCollector
    {
        public ChromDataCollector(double precursorMz, ChromDataCollectorSet chromMap)
        {
            ...
            // Set shared times as long as it is not SRM data
            if (!chromMap.IsSrm)
...
//        private const double MILLION = 1000000;
        ...
        public IEnumerable<FilteredSrmSpectrum> SrmSpectraFromFullScan(double? time,
            double? isolationTargetMz, double? isolationWidth, double[] mzArray, double[] intensityArray)
        {
            if (!EnabledMsMs || !time.HasValue || !isolationTargetMz.HasValue || mzArray == null || intensityArray == null)
                yield break;

            foreach (var filterPair in FindFilterPairs(isolationTargetMz.Value, isolationWidth, _precursorFilterType))
            {
            ...
            FullScanPrecursorFilterType precursorFilterType)
        {
            if (precursorFilterType == FullScanPrecursorFilterType.Multiple)
            {
                ...
                while (iFilter < _filterMzValues.Count && CompareMz(isolationTargetMz,
        ...
        {
            return IndexOfFilter(precursorMz, window, 0, _filterMzValues.Count - 1);
        }
