...
namespace pwiz.Skyline.Model.Results
{
    ...
    internal sealed class SpectraChromDataProvider : ChromDataProvider
    {
            ...
            : base(status, startPercent, endPercent, loader)
        {
            ...
            for (int i = 0; i < lenSpectra; i++)
            {
                ...
                if (dataFile.GetSrmSpectrum(i, filter.Enabled, out time, out precursorMz, out mzArray, out intensityArray))
                {
                    if (!time.HasValue)
                    ...
                    ProcessSrmSpectrum(time.Value, precursorMz.Value, mzArray, intensityArray, chromMap);
                }
                else if (time.HasValue)
                {
                    // Process all SRM spectra that can be generated by filtering this full scan MS/MS
                    foreach (var spectrum in filter.SrmSpectraFromFullScan(time, precursorMz, mzArray, intensityArray))
                    {
                        ProcessSrmSpectrum(time.Value, spectrum.PrecursorMz,
    ...
    internal sealed class SpectrumFilter
    {
        ...
        private readonly double _productFilterWindow;
        ...
        public SpectrumFilter(SrmDocument document)
        {
            ...
            _precursorFilterType = instrument.PrecursorFilterType;
            if (Enabled)
            {
                if (instrument.PrecursorFilter.HasValue)    // Keep resharper happy
                    _precursorFilterWindow = instrument.PrecursorFilter.Value;

                _isHighAccProductFilter = Equals(instrument.ProductFilterType,
                    FullScanProductFilterType.HIGH_ACCURACY);

                if (instrument.ProductFilter.HasValue)      // Key resharper happy
                    _productFilterWindow = instrument.ProductFilter.Value;
                ...
                foreach (var nodeGroup in document.TransitionGroups)
                {
                    ...
                    var listQ3 = (from TransitionDocNode nodeTran in nodeGroup.Children select nodeTran.Mz).ToList();
                    listQ3.Sort();
                    filter.ArrayQ3 = listQ3.ToArray();
        ...
        public bool Enabled { get { return _precursorFilterType != FullScanPrecursorFilterType.None; } }
        ...
        {
            if (!Enabled || !time.HasValue || !precursorMz.HasValue || mzArray == null || intensityArray == null)
                yield break;

            foreach (var filterPair in FindFilterPairs(precursorMz.Value, _precursorFilterType))
            {
                double[] mzArrayNew = filterPair.ArrayQ3;
                double[] intensityArrayNew = new double[mzArrayNew.Length];

                // Search for matching peaks for each Q3 filter
                int iPeak = 0;
                for (int i = 0; i < mzArrayNew.Length; i++)
                {
                    // TODO: Support FullScanProductFilterType.HIGH_ACCURACY by handling _productFilterWindow as ppm
                    if (_isHighAccProductFilter)
                        throw new InvalidDataException("Full-scan MS/MS filtering of high accuracy spectra is not yet supported.  Send us your data.");

                    // Look for the first peak that is greater than the start of the filter
                    double startFilter = mzArrayNew[i] - _productFilterWindow / 2;
                    while (iPeak < mzArray.Length && mzArray[iPeak] < startFilter)
                        iPeak++;

                    // TODO: Support FullScanProductFilterType.HIGH_ACCURACY by ignoring intensity that is not close enough to the peak apex

                    // Add the intensity values of all peaks less than the end of the filter
                    int iNext = iPeak;
                    double endFilter = startFilter + _productFilterWindow;
                    while (iNext < mzArray.Length && mzArray[iNext] < endFilter)
                        intensityArrayNew[i] += intensityArray[iNext++];
                }

                yield return new FilteredSrmSpectrum(filterPair.Q1, mzArrayNew, intensityArrayNew);
            }
        }
