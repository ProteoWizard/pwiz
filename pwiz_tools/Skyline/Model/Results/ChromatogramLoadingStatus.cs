    ...
    public class ChromatogramLoadingStatus : ProgressStatus
    {
        ...
        public class TransitionData
        {
            ...
            public void Add(int filterIndex, ChromSource chromSource, float time, float[] intensities)
            {
                ...
                if (peak == null)
                    _peaks[filterIndex][sourceIndex] = peak = new Peak(filterIndex);
                ...
                if (lastIndex >= 0)
                {
                    ...
                    if (peak.MaxIntensity < lastIntensity)
                    {
                        ...
                        _threshold = Math.Max(_threshold, peak.MaxIntensity*THRESHOLD);
                    }

                    // Finish a peak if intensity falls too low.
                    if (lastIntensity < _threshold)
                    {
                        // If the peak was above our current threshold, send to display.
                        if (peak.Intensities.Count > 1 && peak.MaxIntensity > _threshold)
                        {
                            lock (this)
                            {
            ...
            public void AddSrm(int index, float[] times, float[] intensities)
            {
                ...
                int startIndex = 0;
                while (startIndex < intensities.Length && intensities[startIndex] < _threshold)
                    startIndex++;
                ...
                while (endIndex > startIndex && intensities[endIndex] < _threshold)
                    endIndex--;
                if (endIndex < startIndex + 2)
                    return;
                endIndex++;

                var peak = new Peak(index);
