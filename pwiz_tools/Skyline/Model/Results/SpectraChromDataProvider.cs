...
namespace pwiz.Skyline.Model.Results
{
    internal sealed class SpectraChromDataProvider : ChromDataProvider
    {
        ...
        private void ExtractChromatogramsLocked()
        {
            ...
            var dictPrecursorMzToIndex = new Dictionary<SignedMz, SpectrumFilterPair>(); // For SRM processing
            ...
            while (_spectra.NextSpectrum())
            {
                ...
                if (_spectra.HasSrmSpectra)
                {
                    ...
                    if (!dictPrecursorMzToIndex.TryGetValue(precursorMz, out var filterPair))
                    {
                        var peptideNode = peptideFinder != null ? peptideFinder.FindPeptide(precursorMz) : null;
                        var precursorTextId = new PrecursorTextId(precursorMz, null, null, null,
                            ChromatogramGroupId.ForPeptide(peptideNode, null), ChromExtractor.summed);
                        filterPair = new SpectrumFilterPair(precursorTextId,
                            peptideNode?.Color ?? PeptideDocNode.UNKNOWN_COLOR, dictPrecursorMzToIndex.Count, null,
                            null, false, false);
                        dictPrecursorMzToIndex.Add(precursorMz, filterPair);
                    }
                                               ...
                                               ChromDataCollectorSet chromMap)
        {
            ...
            var productFilters = mzs.Select(mz => new SpectrumProductFilter(new SignedMz(mz, filterPair.Q1.IsNegative), 0, 0)).ToArray();
            var spectrum = new ExtractedSpectrum(filterPair, productFilters, intensityFloats, null);
