                ...
                else if (_filter.EnabledMsMs || _filter.EnabledMs)
                {
                    ...
                    if (_filter.IsMsSpectrum(dataSpectrum))
                    {
                        ...
                        lock (_blockWriter)
                        {
                            ...
                            var scanIdIndex = GetScanIdIndex(scanId);
                            foreach (var spectrum in _filter.SrmSpectraFromMs1Scan(rt, dataSpectrum.Precursors, spectra))
                            {
                                addedPoint |= chromMapMs.ProcessExtractedSpectrum(rt, _collectors, scanIdIndex, spectrum, AddChromCollector);
                            }
                            if (chromMapMs.IsSharedTime && addedPoint)
                            {
                                chromMapMs.AddSharedTime(rt, scanIdIndex);
                            }
VerifyTimesCount(chromMapMs);
                        }
                    }
...
private static void VerifyTimesCount(ChromDataCollectorSet chromMapMs)
{
    if (chromMapMs.IsSharedTime)
    {
        foreach (var pcm in chromMapMs.PrecursorCollectorMap)
        {
            if (pcm != null)
                foreach (var pim in pcm.Item2.ProductIntensityMap)
                    if (Math.Abs(pim.Value.Count - chromMapMs.SharedTimesCollector.Count) !=0)
        ...
        /// but possibly a set of drift bins all with same retention time and isolation window,
            ...
            private SpectrumInfo ReadSpectrum(ref int i)
            {
                while ((i = _lookaheadContext.NextIndex(i)) < _countSpectra)
                {
                    ...
                    else
                    {
                        ...
                        var imCheck = _filter.HasIonMobilityRange && _dataFile.GetIonMobilityIsInexpensive ?
                                      _lookaheadContext.GetIonMobility(i) :
        ...
        private struct LookaheadContext
        {
            ...
            private double? _rt;
            private MsPrecursor[] _precursors;
            private readonly SpectrumFilter _filter;
            ...
            private bool NextSpectrumIsIonMobilityScanForCurrentRetentionTimeAndIsolationWindow(MsDataSpectrum nextSpectrum)
            {
                bool result = ((_rt ?? 0) == (nextSpectrum.RetentionTime ?? -1)) &&
                              (nextSpectrum.Level==1 ||  EqualsDeep(_precursors, nextSpectrum.Precursors)) && // For diaPASEF, looking for runs of MS2 with same MS1 ion mobility
            ...
            private static bool EqualsDeep(IReadOnlyList<MsPrecursor> left, IReadOnlyList<MsPrecursor> right)
            {
                if (left == null)
                {
                    return right == null;
            ...
            // Deal with ion mobility data - look ahead for a run of scans all
            // with the same retention time and isolation window. For non-IMS data we'll just get
            // a single "ion mobility bin" with no ion mobility value.
            //
            // Also for Agilent ramped-CE msE, gather MS2 scans together
            // so they get averaged.
    ...
    internal sealed class ChromDataCollectorSet
    {
        ...
        // Returns true if a point gets added to the collector
        public bool ProcessExtractedSpectrum(float time, SpectraChromDataProvider.Collectors chromatograms, int scanId, ExtractedSpectrum spectrum, Action<int, ChromCollector> addCollector)
        {
            bool addedPoint = false;
