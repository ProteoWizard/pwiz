...
namespace pwiz.Skyline.Model.DocSettings.AbsoluteQuantification
{
    public class CalibrationCurveFitter
    {
        ...
        {
            return GetPeptideConcentration(new CalibrationPoint(replicateIndex));
        }
        ...
        public FiguresOfMerit GetFiguresOfMerit(CalibrationCurve calibrationCurve, List<ImmutableList<PointPair>> bootstrapCurves = null)
        {
            ...
            foreach (int replicateIndex in EnumerateReplicates())
            {
                ...
                {
                    var peakArea = GetNormalizedPeakArea(new CalibrationPoint(replicateIndex));
        ...
        private QuantificationResult GetPeptideQuantificationResult(int replicateIndex)
        {
            ...
            result = result.ChangeNormalizedArea(GetAnnotatedNormalizedPeakArea(new CalibrationPoint(replicateIndex)));
            if (HasExternalStandards() || HasInternalStandardConcentration())
            {
                AnnotatedDouble calculatedConcentration = GetAnnotatedConcentration(calibrationCurve, new CalibrationPoint(replicateIndex));
        ...
        {
            return new QuantificationResult().ChangeNormalizedArea(AnnotatedDouble.Of(GetMultiplexArea(replicateIndex, multiplexName)));
        }
