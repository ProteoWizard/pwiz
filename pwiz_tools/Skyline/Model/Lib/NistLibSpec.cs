...
// ReSharper restore InconsistentNaming
    ...
    public sealed class NistLibrary : NistLibraryBase
    {
        ...
        public override LibraryDetails LibraryDetails
        {
            get
            {
                LibraryDetails details = new LibraryDetails { Format = "NIST", SpectrumCount = SpectrumCount }; // Not L10N
...
// ReSharper restore NonLocalizedString
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    while ((line = reader.ReadLine()) != null)
                    {
                        ...
                        if (!isPeptide)
                        {
                            ...
                            if (match.Success)
                            {
                                CAS = MoleculeAccessionNumbers.TagCAS+":"+match.Groups[1].Value; // Not L10N
                                continue;
                            }
                    ...
                    for (int i = 0; i < numPeaks; i++)
                    {
                        ...
                        int iTab2 = (iTab1 == -1 ? -1 : line.IndexOf(sep, iTab1 + 1)); // Not L10N
                        if (iTab2 == -1)
                        {
                            if (iTab1 != -1)
                            {
        ...
        public override IEnumerable<double> GetRetentionTimesWithSequences(
            MsDataFileUri filePath, IEnumerable<Target> peptideSequences, ref int? fileIndex)
        {
            if (!fileIndex.HasValue)
            {
                fileIndex = FindFileInList(filePath, new[] {FilePath});
            }
            if (fileIndex.Value < 0)
            {
                return new double[0];
            }
            return LibraryEntriesWithSequences(peptideSequences).Where(spectrum => spectrum.RT.HasValue)
                .Select(spectrum => spectrum.RT.Value);
        }
