...
namespace pwiz.Skyline.Model
{
    ...
    public partial class AreaCVGraphData : Immutable
    {
            ...
            CancellationToken? token = null, AreaCVRefinementSettings refineSettings = null)
        {
            ...
            if (_isGraphData)
            {
                annotations = AnnotationHelper.GetPossibleAnnotations(document.Settings, graphSettings.Group, AnnotationDef.AnnotationTarget.replicate);
            ...
            Data = ImmutableList<CVData>.ValueOf(data.GroupBy(i => i, (key, grouped) =>
            {
                ...
                return new CVData(
                        ...
                        key.CVBucketed.HasValue ? key.CVBucketed.Value : key.CV, key.Area, groupedArray.Length);
            }).OrderBy(d => d.CV));
        ...
        private MedianInfo CalculateMedianAreas(SrmDocument document)
        {
            ...
            else if (!double.IsNaN(_refineSettings.QValueCutoff) &&
