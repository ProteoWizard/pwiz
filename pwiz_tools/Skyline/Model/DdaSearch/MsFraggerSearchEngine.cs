            ...
            using (var fixedPepXmlFile = new StreamWriter(finalOutputFilepath))
            {
                ...
                while ((line = pepXmlFile.ReadLine()) != null)
                {
                    if (line.Contains(@"base_name"))
                        line = Regex.Replace(line, "base_name=\"NA\"", $"base_name=\"{XmlUtil.EscapePath(spectrumFilename.GetFileNameWithoutExtension())}\"");
                    if (line.Contains(@"search_database"))
                        line = Regex.Replace(line, "search_database local_path=\"\\(null\\)\"", $"search_database local_path=\"{XmlUtil.EscapePath(_fastaFilepath)}\"");
