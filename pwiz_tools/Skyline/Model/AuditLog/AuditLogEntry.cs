...
namespace pwiz.Skyline.Model.AuditLog
{
    ...
    public class AuditLogList : Immutable, IXmlSerializable
    {
        ...
        /// <summary>
        /// Ensure that the "EnExtraInfo" property in each AuditLogEntry is the correct string
        /// generated by the current version of Skyline.
        /// </summary>
        public AuditLogList RecomputeEnExtraInfos()
        {
            AuditLogEntry parentEntry = AuditLogEntry.ROOT;
            foreach (var auditLogEntry in AuditLogEntries.Enumerate().Reverse())
            {
                parentEntry = auditLogEntry.RecomputeEnExtraInfo().ChangeParent(parentEntry);
            }

            return ChangeProp(ImClone(this), im => im.AuditLogEntries = parentEntry);
        }
    ...
    public class AuditLogEntry : Immutable, IXmlSerializable
    {
        ...
        public string EnExtraInfo
        {
            get
            {
                if (_enExtraInfo == null)
                {
                    _enExtraInfo = ComputeEnExtraInfo();
                }
                return _enExtraInfo;
            }
