            ...
            IModifyDocumentContainer
    {
        ...
        public void ModifyDocument(string description, [InstantHandle] Func<SrmDocument, SrmDocument> act)
        {
            ...
            ModifyDocument(description, null, act, null, null,
                docPair => AuditLogEntry.CreateSimpleEntry(MessageType.test_only, docPair.NewDoc.DocumentType, description ?? string.Empty));
        }
...
//            Enzyme enzyme = settings.PeptideSettings.Enzyme;
        ...
        public static AuditLogEntry CreateDeleteNodesEntry(SrmDocumentPair docPair, IEnumerable<string> items, int? count)
        {
            var entry = AuditLogEntry.CreateCountChangeEntry(MessageType.deleted_target,
                MessageType.deleted_targets, docPair.NewDoc.DocumentType, items, count);
...
//            }
       ...
       public void ShowPastePeptidesDlg()
       {
        ...
        public static int CountNodeDiff(SrmDocumentPair docPair)
        {
            ...
            var enumerator = Reflector<Targets>.EnumerateDiffNodes(objInfo, property, docPair.DocumentTypeForAuditLogging, false);
        ...
        private AuditLogEntry CreateRemoveNodesEntry(SrmDocumentPair docPair, MessageType singular, MessageType plural)
        {
            ...
            return AuditLogEntry.CreateSimpleEntry(count == 1 ? singular : plural, docPair.NewDoc.DocumentType, count);
        }
        ...
        public void RemoveMissingResults()
        {
            ...
            ModifyDocument(Resources.SkylineWindow_RemoveMissingResults_Remove_missing_results, refinementSettings.Refine,
                docPair => AuditLogEntry.CreateSimpleEntry(MessageType.removed_missing_results, docPair.NewDoc.DocumentType));
        }
        ...
        private void PerformSort(string title, Comparison<PeptideGroupDocNode> comparison, MessageType type)
        {
            ...
            }, docPair => AuditLogEntry.CreateSingleMessageEntry(new MessageInfo(type, docPair.NewDoc.DocumentType)));
        }
                ...
                if (!string.IsNullOrEmpty(labelText))
                {
                    ...
                    if (oldPeptideGroupDocNode == null)
                    {
                        ...
                        }, docPair =>
                        {
                            ...
                            return AuditLogEntry.CreateSingleMessageEntry(new MessageInfo(type, docPair.NewDoc.DocumentType, peptideGroupName), labelText).Merge(entry);
                        });
                    }
                    else
                    {
            ...
            else if (!e.CancelEdit)
            {
                ...
                if (nodeTree != null && e.Label != null && !Equals(nodeTree.Text, e.Label))
                {
                    ModifyDocument(
                        ...
                        docPair => AuditLogEntry.CreateSimpleEntry(MessageType.renamed_node, docPair.NewDoc.DocumentType,
            ...
            ModifyDocument(
                ...
                docPair =>
                {
                    ...
                    var entry = AuditLogEntry.CreateCountChangeEntry(MessageType.picked_child,
                        MessageType.picked_children, docPair.NewDoc.DocumentType, chosen,
                        n => MessageArgs.Create(n.AuditLogText, nodeName),
                        MessageArgs.Create(chosen.Length, nodeName));

                    return entry.AppendAllInfo(chosen.Select(n => new MessageInfo(MessageType.picked_child, docPair.NewDoc.DocumentType,
        ...
        private void sequenceTree_DragDrop(object sender, DragEventArgs e)
        {
            ...
            {
                var entry = AuditLogEntry.CreateCountChangeEntry(MessageType.drag_and_dropped_node, MessageType.drag_and_dropped_nodes, docPair.OldDoc.DocumentType,
