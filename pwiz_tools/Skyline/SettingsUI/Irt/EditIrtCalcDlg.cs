...
namespace pwiz.Skyline.SettingsUI.Irt
{
    public partial class EditIrtCalcDlg : FormEx
    {
        ...
        public void OpenDatabase(string path)
        {
            ...
            using (var dlg = new LongWaitDlg { Message = IrtResources.EditIrtCalcDlg_OpenDatabase_Opening_database })
            {
            ...
            try
            {
                ...
                if (DatabaseChanged)
                {
                    using (var fileSaver = new FileSaver(path))
                    {
                        ...
                        using (var longWaitDlg = new LongWaitDlg { Text = IrtResources.EditIrtCalcDlg_OkDialog_Saving_to_database })
                        {
                            var dbCopy = db;
        ...
        private class LibraryGridViewDriver : PeptideGridViewDriver<DbIrtPeptide>
        {
            ...
            public void AddResults()
            {
                ...
                using (var longWait = new LongWaitDlg
                       {
                           Text = IrtResources.LibraryGridViewDriver_AddResults_Adding_Results,
                           Message = IrtResources.LibraryGridViewDriver_AddResults_Adding_retention_times_from_imported_results,
            ...
            private void AddSpectralLibrary(LibrarySpec librarySpec)
            {
                ...
                try
                {
                    ...
                    using (var longWait = new LongWaitDlg
                           {
                               Text = IrtResources.LibraryGridViewDriver_AddSpectralLibrary_Adding_Spectral_Library,
                               Message = string.Format(IrtResources.LibraryGridViewDriver_AddSpectralLibrary_Adding_retention_times_from__0__, librarySpec.FilePath),
                               FormBorderStyle = FormBorderStyle.Sizable
                           })
                    {
                        try
            ...
            private void AddIrtDatabase(RCalcIrt irtCalc)
            {
                ...
                using (var longWait = new LongWaitDlg
                       {
                           Text = IrtResources.LibraryGridViewDriver_AddIrtDatabase_Adding_iRT_Database,
                           Message = string.Format(IrtResources.LibraryGridViewDriver_AddSpectralLibrary_Adding_retention_times_from__0__, irtCalc.DatabasePath),
                           FormBorderStyle = FormBorderStyle.Sizable
                       })
                {
                    try
                               ...
                               MultiButtonMsgDlg.BUTTON_YES, MultiButtonMsgDlg.BUTTON_NO, true))
                    {
                        switch (dlg.ShowDialog(MessageParent))
                        {
                            ...
                            case DialogResult.Yes:
                                using (var longWait = new LongWaitDlg
                                       {
                                           Text = IrtResources.LibraryGridViewDriver_AddToLibrary_Recalibrate_iRT_Standard_Peptides,
                                           Message = IrtResources.LibraryGridViewDriver_AddToLibrary_Recalibrating_iRT_standard_peptides_and_reprocessing_iRT_values
                                       })
                                {
                                    try
                                    {
