    ...
    public partial class ChangeIrtPeptidesDlg : FormEx
    {
        ...
        public void OkDialog()
        {
            ...
            if (invalidLines.Count > 0)
            {
                ...
                {
                    message = string.Format(ModeUIHelper.Translate(Resources.ChangeIrtPeptidesDlg_OkDialog_The_sequence__0__is_not_currently_in_the_database),
                                            invalidLines[0]);
                    MessageBox.Show(this, message, Program.Name);
                }
                else
                {
                    message = TextUtil.LineSeparate(ModeUIHelper.Translate(Resources.ChangeIrtPeptidesDlg_OkDialog_The_following_sequences_are_not_currently_in_the_database),
                                                    string.Empty,
                                                    TextUtil.LineSeparate(invalidLines),
                                                    string.Empty,
                                                    ModeUIHelper.Translate(Resources.ChangeIrtPeptidesDlg_OkDialog_Standard_peptides_must_exist_in_the_database));
                    MessageBox.Show(this, message, Program.Name);
                }
                return;
            }
