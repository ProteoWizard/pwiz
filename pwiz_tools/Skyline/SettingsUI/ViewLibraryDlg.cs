            ...
            using (var addModificationsDlg = new AddModificationsDlg(Document.Settings, _selectedLibrary))
            {
                ...
                if (addModificationsDlg.NewDocumentModsStatic.Any() || addModificationsDlg.NewDocumentModsHeavy.Any())
                {
                    Program.MainWindow.ModifyDocument(Resources.ViewLibraryDlg_MatchModifications_Add_modifications, doc =>
                    {
                        ...
                        .ChangeUndoRedo(new MessageInfo(MessageType.matched_modifications_of_library, docPair.NewDoc.DocumentType,
                            _selectedLibName)));
                }
                return true;
            }
...
//            var pepInfo = (ViewLibraryPepInfo)lPeptide.SelectedItem;
        ...
        {
            return AuditLogEntry.CreateSimpleEntry(type, docPair.NewDoc.DocumentType, args).Merge(docPair, entryCreatorList);
        }
            ...
            try
            {
                ...
                using (var longWaitDlg = new LongWaitDlg())
                {
                    ...
                    if (finished && entryCreators != null)
                        entryCreators.Add(docPair => AuditLogEntry.CreateSimpleEntry(MessageType.upgraded_background_proteome, docPair.NewDoc.DocumentType, backgroundProteome.Name));
                    return finished;
                } 
            }
            catch (Exception e)
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    if (addToDoc)
                        Program.MainWindow.ModifyDocument(Resources.ViewLibraryDlg_CheckLibraryInSettings_Add_Library, oldDoc => copy,
                            docPair => AuditLogEntry.CreateSimpleEntry(MessageType.added_spectral_library, docPair.NewDoc.DocumentType, _selectedLibName));
                }
