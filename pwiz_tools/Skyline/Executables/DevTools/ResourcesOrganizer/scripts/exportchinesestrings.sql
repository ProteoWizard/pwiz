SELECT Id, Name, Comment, English, Chinese, ChineseIssue, OriginalEnglish, FileCount, CASE WHEN FileCount = 1 THEN FirstFile END AS File FROM (
SELECT i.Id, i.Name, i.Comment, i.Value as English, zh.Value as Chinese, zh.Problem as ChineseIssue, zh.OriginalInvariantValue as OriginalEnglish, (SELECT MIN(FilePath) FROM ResourceLocation loc INNER JOIN ResxFile f ON loc.ResxFileId = f.Id WHERE loc.InvariantResourceId = i.Id) AS FirstFile, (SELECT COUNT(Id) FROM ResourceLocation loc WHERE loc.InvariantResourceId = i.Id) as FileCount
