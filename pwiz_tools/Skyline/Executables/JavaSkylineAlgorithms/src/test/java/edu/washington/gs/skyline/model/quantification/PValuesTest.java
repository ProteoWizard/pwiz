...
public class PValuesTest extends TestCase {
    public void testSimpleAdjustPValues()
    {
        double[] input = {.005, .02, .25};
        double[] expected = {.015, .030, .250};
        double[] actual = PValues.adjustPValues(input);
    ...
    */
    public void testAdjustListOfPValues()
    {
        ...
        for (int i = 0; i < expected.length; i++)
        {
            ...
            {
                assertEquals(expected[i], actual[i], expected[i] / 1E8);
            }
        }
    }
