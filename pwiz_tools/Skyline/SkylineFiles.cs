    ...
    public partial class SkylineWindow
    {
        ...
        public bool OpenFile(string path, FormEx parentWindow = null)
        {
            ...
            try
            {
                ...
                {
                    Text = SkylineResources.SkylineWindow_OpenFile_Loading___,
            ...
            var settings = document.Settings.ConnectLibrarySpecs((library, librarySpec) =>
                {
                    ...
                    using (var dlg = new MissingFileDlg
                                  {
                                      ItemName = name,
                                      ItemType = SkylineResources.SkylineWindow_ConnectLibrarySpecs_Spectral_Library,
                                      Filter = library != null ? library.SpecFilter : librarySpec.Filter,
                                      FileHint = fileName,
                                      FileDlgInitialPath = Path.GetDirectoryName(documentPath),
                                      Title = SkylineResources.SkylineWindow_ConnectLibrarySpecs_Find_Spectral_Library
                                  })
                    {
                ...
                using (var dlg = new MissingFileDlg
                         {
                             ...
                             ItemType = SkylineResources.SkylineWindow_FindIrtDatabase_iRT_Calculator,
                             Filter = TextUtil.FileDialogFilterAll(SkylineResources.SkylineWindow_FindIrtDatabase_iRT_Database_Files, IrtDb.EXT),
                             FileHint = Path.GetFileName(irtCalc.DatabasePath),
                             FileDlgInitialPath = Path.GetDirectoryName(documentPath),
                             Title = SkylineResources.SkylineWindow_FindIrtDatabase_Find_iRT_Calculator
                         })
                {
                    if (dlg.ShowDialog(parent) == DialogResult.OK)
                    {
        ...
        private OptimizationLibrary FindOptimizationDatabase(IWin32Window parent, string documentPath, OptimizationLibrary optLib)
        {
            ...
            do
            {
                using (var dlg = new MissingFileDlg
                {
                    ...
                    ItemType = SkylineResources.SkylineWindow_FindOptimizationDatabase_Optimization_Library,
                    Filter = TextUtil.FileDialogFilterAll(SkylineResources.SkylineWindow_FindOptimizationDatabase_Optimization_Library_Files, OptimizationDb.EXT),
                    FileHint = Path.GetFileName(optLib.DatabasePath),
                    FileDlgInitialPath = Path.GetDirectoryName(documentPath),
                    Title = SkylineResources.SkylineWindow_FindOptimizationDatabase_Find_Optimization_Library
                })
                {
                    if (dlg.ShowDialog(parent) == DialogResult.OK)
                    {
        ...
        private IonMobilityLibrary FindIonMobilityLibrary(IWin32Window parent, string documentPath, IonMobilityLibrary ionMobilityLibrary)
        {
            ...
            do
            {
                using (var dlg = new MissingFileDlg
                {
                    ...
                    ItemType = SkylineResources.SkylineWindow_FindIonMobilityLibrary_Ion_Mobility_Library,
                    Filter = TextUtil.FileDialogFilterAll(SkylineResources.SkylineWindow_FindIonMobilityDatabase_ion_mobility_library_files, IonMobilityDb.EXT),
                    FileHint = Path.GetFileName(ionMobilityLibrary.FilePath),
                    FileDlgInitialPath = Path.GetDirectoryName(documentPath),
                    Title = SkylineResources.SkylineWindow_FindIonMobilityLibrary_Find_Ion_Mobility_Library
                })
                {
                    if (dlg.ShowDialog(parent) == DialogResult.OK)
                    {
            ...
            using (var dlg = new MissingFileDlg
                    {
                        ...
                        ItemType = SkylineResources.SkylineWindow_FindBackgroundProteome_Background_Proteome,
                        Filter = TextUtil.FileDialogFilterAll(SkylineResources.SkylineWindow_FindBackgroundProteome_Proteome_File, ProteomeDb.EXT_PROTDB),
                        FileDlgInitialPath = Settings.Default.ProteomeDbDirectory,
                        Title = SkylineResources.SkylineWindow_FindBackgroundProteome_Find_Background_Proteome
                    })
            {
                if (dlg.ShowDialog(parent) == DialogResult.OK)
                {
        ...
        public bool SaveDocument(String fileName, bool includingCacheFile = true)
        {
            ...
            try
            {
                ...
                if (includingCacheFile)
                {
                    using (var longWaitDlg = new LongWaitDlg(this)
                    {
                        Text = SkylineResources.SkylineWindow_SaveDocument_Optimizing_data_file___,
                        Message = Path.GetFileName(fileName)
                    })
                    {
        ...
        private string GetShareFileName()
        {
            using var dlg = new SaveFileDialog
            {
                Title = SkylineResources.SkylineWindow_shareDocumentMenuItem_Click_Share_Document,
                OverwritePrompt = true,
        ...
        public bool ShareDocument(string fileDest, ShareType shareType)
        {
            ...
            {
                bool success;
                using (var longWaitDlg = new LongWaitDlg { Text = SkylineResources.SkylineWindow_ShareDocument_Compressing_Files, })
                {
        ...
        public void ShowExportSpectralLibraryDialog()
        {
            ...
            using (var dlg = new SaveFileDialog
            {
                Title = SkylineResources.SkylineWindow_ShowExportSpectralLibraryDialog_Export_Spectral_Library,
                OverwritePrompt = true,
            ...
            })
            {
                ...
                try
                {
                    using (var longWaitDlg = new LongWaitDlg
                    {
                        Text = SkylineResources.SkylineWindow_ShowExportSpectralLibraryDialog_Export_Spectral_Library,
                        Message = string.Format(SkylineResources.SkylineWindow_ShowExportSpectralLibraryDialog_Exporting_spectral_library__0____, Path.GetFileName(dlg.FileName))
                    })
                    {
        ...
        public void ShowExportEspFeaturesDialog()
        {
            ...
            using (var dlg = new SaveFileDialog
            {
                Title = SkylineResources.SkylineWindow_ShowExportEspFeaturesDialog_Export_ESP_Features,
                OverwritePrompt = true,
                DefaultExt = EspFeatureCalc.EXT,
                Filter = TextUtil.FileDialogFilterAll(SkylineResources.SkylineWindow_ShowExportEspFeaturesDialog_ESP_Feature_Files,EspFeatureCalc.EXT),
            })
            {
                if (!string.IsNullOrEmpty(DocumentFilePath))
                {
        ...
        private void peakBoundariesToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            using (OpenFileDialog dlg = new OpenFileDialog
            {
                Title = SkylineResources.SkylineWindow_ImportPeakBoundaries_Import_PeakBoundaries,
                CheckPathExists = true
            })
            {
                if (dlg.ShowDialog(this) == DialogResult.OK)
        ...
        private void importAssayLibraryMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                Title = SkylineResources.SkylineWindow_importAssayLibraryMenuItem_Click_Import_Assay_Library,
                InitialDirectory = Settings.Default.ActiveDirectory,
                CheckPathExists = true,
                SupportMultiDottedExtensions = true,
        ...
        private void importMassListMenuItem_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog dlg = new OpenFileDialog
            {
                Title = SkylineResources.SkylineWindow_importMassListMenuItem_Click_Import_Transition_List_title,
            ...
            out MassListInputs irtInputs, out RetentionTimeRegression retentionTimeRegressionStore)
        {
            ...
            if (!assayLibrary)
            {
                ...
                var yesButton = calcIrt == null
                    ...
                    : SkylineResources.SkylineWindow_ImportMassList_Add;
            ...
            using (var longWaitDlg = new LongWaitDlg(this) { Text = SkylineResources.SkylineWindow_ImportMassList_Adding_iRT_values_ })
            {
                var newDoc = doc;
            ...
            out BiblioSpecLiteSpec docLibrarySpec, out BiblioSpecLiteLibrary docLibrary, out int indexOldLibrary)
        {
            ...
            using (var blibDb = BlibDb.CreateBlibDb(AssayLibraryFileName))
            {
                ...
                using (var longWaitDlg = new LongWaitDlg(this) { Text = SkylineResources.SkylineWindow_ImportMassListIntensities_Creating_Spectral_Library })
                {
                    var docNew = doc;
        ...
        private void importDocumentMenuItem_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog dlg = new OpenFileDialog
            {
                Title = SkylineResources.SkylineWindow_importDocumentMenuItem_Click_Import_Skyline_Document,
        ...
        public void ImportFiles(params string[] filePaths)
        {
            ...
            using (var longWaitDlg = new LongWaitDlg(this)
                {
                    Text = SkylineResources.SkylineWindow_ImportFiles_Import_Skyline_document_data,
                })
            {
                longWaitDlg.PerformWork(this, 1000, longWaitBroker =>
            ...
            if (servers.Count == 0)
            {
                DialogResult buttonPress = MultiButtonMsgDlg.Show(
                    ...
                    TextUtil.LineSeparate(
                        ...
                        SkylineResources.SkylineWindow_ShowPublishDlg_Press_Register_to_register_for_a_project_on_PanoramaWeb_,
