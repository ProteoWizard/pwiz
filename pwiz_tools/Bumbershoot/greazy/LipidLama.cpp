...
bool sortByScore(double &lhs, double &rhs)
{
	return lhs > rhs;
}
bool sortByScoreLtoH(double &lhs, double &rhs)
{
	return lhs < rhs;
}
...
int main(int argc, const char* argv[]){
	...
	string cut = argv[2];
	string off = "0." + cut;
	double cutoff = stod(off);

	string top = argv[3];
	//string percent = "0." + top;
	double topPercent = stod(top);
	...
	vector<vector<double>> distros0(21);
	vector<vector<double>> distros1(21);
	vector<vector<string>> distros1lipid(21);
	vector<vector<double>> distros2(21);
	vector<vector<double>> parameters(21);
			...
			else
			{
				...
				else
				{
					...
					{					
						...
						if (spectra[spectra.size()-1].lipidIdent.substr(0, 2) == "PE" && spectra[spectra.size()-1].mod != "+Na" && spectra[spectra.size()-1].mod != "+K" && spectra[spectra.size()-1].mod != "+Li")
						{
	...
	*/
	...
	for (size_t i=0; i<cutoffV.size(); i++)
	{
		...
		{
			int cutoffindex = distros1Temp[i].size()*topPercent/100 - 1;
			if (cutoffindex < 0)
			{
	...
	for (size_t i=0; i<spectra.size(); i++)
	{
		...
		if (spectra[i].lipidIdent.substr(0,2) == "PE" && spectra[i].score >= cutoffV[1] && spectra[i].mod != "+Na" && spectra[spectra.size()-1].mod != "+K" && spectra[spectra.size()-1].mod != "+Li")
		{
			//cout << spectra[i].lipidIdent << "  " << spectra[i].charge << "  " << spectra[i].score << endl;
			lipids.push_back(Lipid());
			lipids[lipids.size()-1].filled = false;
		...
		if (spectra[i].lipidIdent.substr(0,2) == "PE" && spectra[i].score >= cutoffV[12] && (spectra[i].mod == "+Na" || spectra[spectra.size()-1].mod == "+K" || spectra[spectra.size()-1].mod == "+Li"))
		{
			//cout << spectra[i].lipidIdent << "  " << spectra[i].charge << "  " << spectra[i].score << endl;
			lipids.push_back(Lipid());
			lipids[lipids.size()-1].filled = false;
