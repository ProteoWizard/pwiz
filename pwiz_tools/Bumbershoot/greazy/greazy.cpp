...
bool sortByIntensity(MZIntensityPair &lhs, MZIntensityPair &rhs) 
{
	return lhs.intensity > rhs.intensity;
}
bool sortByMZ(Match &lhs, Match &rhs) 
{
	return lhs.mz > rhs.mz;
}
bool sortByMZr(Match &lhs, Match &rhs) 
{
	return lhs.mz < rhs.mz;
}
bool sortByTotalScore(Score &lhs, Score &rhs)
{
	return (lhs.totalScore > rhs.totalScore) || ((lhs.totalScore == rhs.totalScore) && (lhs.intensityScore > rhs.intensityScore));
}
bool sortByName(Score &lhs, Score &rhs)
{
	return lhs.lipidName < rhs.lipidName;
}

vector<vector<Score>> spec, spec2, specAll;
vector<vector<Score>> specSecond;
vector<int> specN;
vector<Secondary> secondary;
vector<vector<double>> firstSecondFull;
int lipCount;

bool sortByHighScore(vector<Score> &lhs, vector<Score> &rhs)
{
	return lhs[0].totalScore > rhs[0].totalScore;
}
bool sortBySpecMZ(SpectraHolder &lhs, SpectraHolder &rhs) 
{
...
int binarySearch(vector<SpectraHolder> spectraHolder, double mass, int min, int max)
{
	...
	{
		mid = (max+min)/2;
		//cout << mid <<  "  " << spectraHolder[mid].preMassN << endl;
 ...
 vector<vector<size_t>> CLspeciesCount;
...
int main(int argc, const char* argv[])
{
	...
	cout << "Greazy: Phospholipid Identificaiton" << endl << endl;

	if (argc < 2 || !bfs::exists(argv[1]))
	{
		return 1;
	}
	...
	*/
	...
	specAll.resize(numSpectra);
	specSecond.resize(numSpectra);
	specN.resize(numSpectra);
	...
	int spectraIndex = 0;
	double retTime;
	for (size_t i=0; i < numSpectra; ++i) // numSpectra 5602 5603
	{
		SpectrumPtr s = SL->spectrum(i);
		if (s->precursors.empty() == true)
		{		
		...
		{	
			...
			//cout << "mass:  " << mass << "   Tolerance range  " << spectraHolder[spectraIndex].preMassN << "  " << spectraHolder[spectraIndex].preMassP << endl;
			spectraIndex++;

		}
	}
	//cout << spectraIndex << endl;
	...
	/*
	ofstream sfile;
	sfile.open("spectra.txt");
	for (size_t i=0; i<spectraHolder.size(); i++)
	{
		sfile << i << "   " << spectraHolder[i].spectraIndex << "   " << spectraHolder[i].preMassN << "   " << spectraHolder[i].preMassP << endl;
	}
	sfile.close();
	*/

	//lipid count
	//double lipCount = lipidCount(paraMap, head, back, sphingoHead, cardioHead);
	//cout << "There are " << lipCount << " lipids in your search space" << endl;
	...
	//cout << " done." << endl;
	//cout << "lipids:  " << count << endl;
	//cout << "precursors:  " << pre << endl;
	...
	it = specSecond.begin();
	while (it != specSecond.end())
	{
		if (it->empty())
		{
			it = specSecond.erase(it);
		}
		else
		{
			++it;
	...
	/*
	// score normalization (peak & intensity)
	cout << endl;
	double topPeak = 1;
	double topIntensity = 1;
	for (size_t i=0; i<spec.size(); i++)
	{ 

		if(spec[i][0].peakScore < topPeak)
		{
			topPeak = spec[i][0].peakScore;
		}

		if (spec[i][0].intensityScore < topIntensity)
		{
			topIntensity = spec[i][0].intensityScore;
		}
	}
	cout << topPeak << "  " << topIntensity << endl;
	double scale = log(topPeak)/log(topIntensity);
	cout << scale << endl;
	for (size_t i=0; i<spec.size(); i++)
	{ 
		for (size_t j=0; j<spec[i].size(); j++)
		{
			spec[i][j].intensityScore = pow(spec[i][j].intensityScore, scale);
		}
	}


	// calculate chi-squared value (after score normalization)
	...
	{ 
		for (size_t j=0; j<spec[i].size(); j++)
		{
			if ((spec[i][j].peakScore == 1) && (spec[i][j].intensityScore == 1))
			{
				spec[i][j].totalScore = 0;
			}
			else
			{
				spec[i][j].totalScore = -2*(log(spec[i][j].peakScore) + log(spec[i][j].intensityScore));
			}
		}
	}
	*/
	...
	vector<vector<Score>>::iterator tit = specAll.begin();
	while (tit != specAll.end())
	{
		if (tit->empty())
		{
			tit = specAll.erase(tit);
		}
		else
		{
			++tit;
	...
	/*
	// print list of all hits
	string name = argv[1];
	name += ".txt";
	ofstream ofile;
	ofile.open(name);

	for (size_t i=0; i<specAll.size(); i++)
	{
		if (specAll[i][0].totalScore > 0)
		{
			int spect = 0;
			for (size_t j=0; j<specAll[i].size(); j++)
			{
				if ((spect == 0))
				{
					ofile << "spectrum number  " << specAll[i][j].spectrumNumber << endl;
					spect = 1;
				}
				if (specAll[i][j].totalScore > 0)
				{
					ofile << "	lipid:  " << specAll[i][j].lipidName << "  mass  " << specAll[i][j].lipidMass	<< "  score:  " << specAll[i][j].totalScore << endl;
	...
	ofile.close();
	/*
	// print list of top hits
	string top = argv[1];
	top += "top.txt";
	ofstream tfile(top);
	for (size_t i=0; i<spec.size(); i++)
	{
		tfile << "spectrum: " << spec[i][0].spectrumNumber << endl
			<< "retention time: " << spec[i][0].retTime << endl << endl;
		for (size_t m=0; m<spec[i][0].ms2list.size(); m++)
		{
			tfile << "m/z: " << spec[i][0].ms2list[m].mz << "  intensity: " << spec[i][0].ms2list[m].intensity << endl;
		}
		tfile << endl;
		...
		for (size_t j=0; j<spec[i].size(); j++)
		{
			...
			{
				tfile << "	matched lipid: " << spec[i][j].lipidName << "	mass: " << spec[i][j].lipidMass << endl
					<< "	precursor: " << spec[i][j].precursorDescription << "	mass: " << spec[i][j].precursorMass << endl
			   		//<< "	peak score:       " << spec[i][j].peakScore << endl
					//<< "	intensity score:  " << spec[i][j].intensityScore << endl
					<< "	score: " << spec[i][j].totalScore << endl << endl;
					//<< "	search space precursor mass:  " << spec[i][j].precursorMass << endl
					//<< "    spectra precursor mass:       " << spec[i][j].spectrumPrecursorMass << endl;
					//<< "    anion ratio:            " << spec[i][j].anionRatio << endl
					//<< "    ketene loss ratio:      " << spec[i][j].keteneLossRatio << endl
					//<< "    acid loss ratio:        " << spec[i][j].acidLossRatio << endl;
					//<< "    Fatty acid score:   " << spec[i][j].compScores[0] << endl
					//<< "    Non-FA score:       " << spec[i][j].compScores[1] << endl;
					//<< "    FA1 score:        " << spec[i][j].compScores[2] << endl
					//<< "    FA2 score:        " << spec[i][j].compScores[3] << endl;

			
				for (size_t k=0; k<spec[i][j].match.size(); k++)
				{
					tfile << "			m/z: " << spec[i][j].match[k].mz << "  match: " << spec[i][j].match[k].match << "  intensity: " << spec[i][j].match[k].intensity 
						<< "\tdescription: " << spec[i][j].match[k].fragmentD << endl; // "  Index: " << spec[i][j].match[k].peakIndex << endl;  // << "  type: " << spec[i][j].match[k].fragmentT << endl;
				}
	...
	*/
	...
	second += "second.txt";
	ofstream secfile(second);
	secfile << setprecision(10);
	...
	for (size_t i=0; i<spec.size(); i++)
	{
		...
		for (size_t j=0; j<spec[i].size(); j++)
		{
			if (spec[i][j].totalScore > scoreTest && spec[i][j].totalScore < spec[i][0].totalScore)
			{
				scoreTest =spec[i][j].totalScore;
			}
		}
		if (scoreTest != 0)
		{
			sec.push_back(scoreTest);
		}
	}
	...
	string second2 = argv[1];
	second2 += "second2.txt";
	ofstream secfile2(second2);
	secfile2 << setprecision(10);

	for (size_t i=0; i<secondary.size(); i++)
	{
		if (secondary[i].PC[1] != 0)
		{
			secfile2 << secondary[i].PC[1] << endl;
		}
		if (secondary[i].PE[1] != 0)
		{
			secfile2 << secondary[i].PE[1] << endl;
		}
		if (secondary[i].PI[1] != 0)
		{
			secfile2 << secondary[i].PI[1] << endl;
		}
		if (secondary[i].PS[1] != 0)
		{
			secfile2 << secondary[i].PS[1] << endl;
		}
		if (secondary[i].PG[1] != 0)
		{
			secfile2 << secondary[i].PG[1] << endl;
		}
		if (secondary[i].PA[1] != 0)
		{
			secfile2 << secondary[i].PA[1] << endl;
		}
		if (secondary[i].SM[1] != 0)
		{
			secfile2 << secondary[i].SM[1] << endl;
		}
		if (secondary[i].CL[1] != 0)
		{
			secfile2 << secondary[i].CL[1] << endl;
		}
		if (secondary[i].CL2[1] != 0)
		{
			secfile2 << secondary[i].CL2[1] << endl;
		}
		if (secondary[i].PIP[1] != 0)
		{
			secfile2 << secondary[i].PIP[1] << endl;
		}
		if (secondary[i].PIP2[1] != 0)
		{
			secfile2 << secondary[i].PIP2[1] << endl;
		}
	}
	*/
	...
	forR += "forR.txt";
	ofstream Rfile(forR);
	Rfile << setprecision(10);

	string forDiff = argv[1];
	forDiff += "forDiff.txt";
	ofstream diffFile(forDiff);
	diffFile << setprecision(10);
	
	for (size_t i=0; i<firstSecondFull.size(); i++)
	{
		diffFile << firstSecondFull[i][0] << "	" << firstSecondFull[i][1] << endl;
	}
	*/
	...
	for (size_t i=0; i<spec2.size(); i++)
	{
		if (spec2[i][0].totalScore > 0)
		{
			/*
			hfile << setprecision(10);
			hfile << spec[i][0].spectrumNumber << "	" << "identifier" << "	" << spec[i][0].lipidName << endl;
			hfile << spec[i][0].spectrumNumber << "	" << "chemical_formula" << "	" << spec[i][0].chemFormula << endl;
			hfile << spec[i][0].spectrumNumber << "	" << "calc_mass_to_charge" << "	" << spec[i][0].precursorMass << endl;			
			hfile << spec[i][0].spectrumNumber << "	" << "exp_mass_to_charge" << "	" << spec[i][0].spectrumPrecursorMass << endl;
			hfile << spec[i][0].spectrumNumber << "	" << "charge" << "	" << spec[i][0].charge << endl;
			hfile << spec[i][0].spectrumNumber << "	" << "retention_time" << "	" << spec[i][0].retTime << endl;
			hfile << spec[i][0].spectrumNumber << "	" << "best_search_engine_score" << "	" << spec[i][0].totalScore << endl;
			hfile << spec[i][0].spectrumNumber << "	" << "modifications" << "	" << spec[i][0].mod << endl;
			*/
			hfile << "---" << endl;
			...
			//Rfile << spec2[i][0].totalScore << endl;
		}
	}
	...
	for (size_t i=0; i<spec.size(); i++)
	{
		for (size_t j=0; j<spec[i].size(); j++)
		{
			if (spec[i][j].totalScore < spec[i][0].totalScore)
			{
				string lipName = spec[i][j].lipidName;



				if (lipName.substr(0, 2) == "PC")
				{
					lipName = "PC";
				}
				if (lipName.substr(0, 2) == "PE")
				{
					lipName = "PE";
				}
				if (lipName.substr(0, 3) == "PI(")
				{
					lipName = "PI";
				}
				if (lipName.substr(0, 2) == "PG")
				{
					lipName = "PG";
				}
				if (lipName.substr(0, 2) == "PS")
				{
					lipName = "PS";
				}
				if (lipName.substr(0, 2) == "PA")
				{
					lipName = "PA";
				}
				if (lipName.substr(0, 2) == "CL" && spec[i][j].mod != "-2H")
				{
					lipName = "CL";
				}
				if (lipName.substr(0, 2) == "CL" && spec[i][j].mod == "-2H")
				...
				if (lipName.substr(0, 2) == "SM")
				{
					lipName = "PC";
				}
				if (lipName.substr(0, 3) == "PIP" && spec[i][j].mod != "-2H")
				{
					lipName = "PIP";
				}
				if (lipName.substr(0, 3) == "PIP" && spec[i][j].mod == "-2H")
				...
				hfile << lipName << " " << spec[i][j].totalScore << endl;
				break;
			}
		}
	}
	*/
	...
	for (size_t i=0; i<secondary.size(); i++)
	{
		...
		if (secondary[i].M3CL[0] != 0)
		{
			hfile << "M3CL " << secondary[i].M3CL[0] << " " << secondary[i].M3CL[1] << endl;
		}
	...
	if (spec.size() > 0)
	{
		int distro = 1+4*spec[0][0].totalScore/2;
		vector<int> distroV(distro);
		vector<double> distroD(distro);
		int totalN = 0;
		for (size_t i=0; i<spec.size(); i++)
		{
			//cout << spec[i][0].totalScore << endl;
			if (2*spec[i][0].totalScore < 1)
			{
				distroV[0]++;
				totalN++;
			}
			if (2*spec[i][0].totalScore >= 1)
			{
				for (size_t j=1; j<distroV.size(); j++)
				{
					if (2*spec[i][0].totalScore >= j && 2*spec[i][0].totalScore < j+1)
					{
						distroV[j]++;
						totalN++;
					}
		...
		for (size_t i=0; i<distroV.size(); i++)
		{
			distroD[i] = (double)distroV[i]/(double)totalN;
	
	
		int ijit = 0;
		string direct0 = argv[1];
		while(direct0[ijit] != '.')
		{
			ijit++;
		}
		...
		for (size_t i=0; i<distroV.size(); i++)
		{
			if (distroV[i] > dHeight)
			{
				dHeight = distroV[i];
			}
		}
		direct0 = direct0.substr(0, ijit);
		stringstream sss;
		sss << direct0 << ".html";

		ofstream histSVG(sss.str());

		histSVG << "<!DOCTYPE html>" << endl << "<html>" << endl << "<body>" << endl << endl
			//<< "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>" << endl
			<< "<svg width=\"" << 1800 << "\" height=\"" << 1000 << "\">" << endl
			<< "<text font-family=\"sans-serif\" font-size=\"18\" transform=\"rotate(270 35,300)\" x=\"35\" y=\"300\" fill=\"black\">Frequency</text>" << endl
		...
		for (size_t k=10; k<dHeight; k+=10)
		{
			histSVG << "<line x1=\"" << 93 << "\" y1=\"" << 500 - k*400/dHeight << "\" x2=\"" << 107 << "\" y2=\"" << 500 - k*400/dHeight << "\" stroke=\"black\" stroke-width=\"1\" />" << endl
				<< "<text font-family=\"sans-serif\" x=\"60\" y=\"" << 506 - k*400/dHeight << "\" fill=\"black\">" << k << "</text>" << endl;
		}

		for (size_t k=10; k<spec[0][0].totalScore + 10; k+=10)
		{
			histSVG << "<line x1=\"" << 100 + k*1600/distro << "\" y1=\"507\" x2=\"" << 100 + k*1600/distro << "\" y2=\"493\" stroke=\"black\" stroke-width=\"1\" />" << endl
				<< "<text text-anchor=\"middle\" font-family=\"sans-serif\" x=\"" << 100 + k*1600/distro << "\" y=\"523\" fill=\"black\">" << k << "</text>" << endl;
		}
		for (size_t k=0; k<distroV.size(); k++)
		{
			histSVG << "<rect x=\"" << 100 + k*800/distro << "\" y=\"" << 500-distroV[k]*400/dHeight << "\" width=\"" << 800/distro << "\" height=\"" << distroV[k]*400/dHeight << "\" stroke=\"black\" fill=\"white\"/>" << endl;
		}

		histSVG	<< "</svg>" << endl << endl
		<< "</body>" << endl << "</html>" << endl;
		histSVG.close();
		}
	}
...
Map processConfig(const string & fname)
{
	...
	string line, tempLine, nestLev[4], pStr;
	Map tempMap;
											...
											if ((u != 0) && (u < paraMap["CL.LowerLengthLim2b"]))
											{
												...
												for (size_t v = paraMap["CL.LowerDoubleBondLim2b"]; v <= paraMap["CL.UpperDoubleBondLim2b"]; v++)
												{
													...
													if (FA1mass != H || FA2mass != H || FA3mass != H || FA4mass != H)
													{
														...
														size_t CLsize = CLspeciesCount.size();
														for (size_t c = 0; c<CLsize; c++)
														{
															if (CLspeciesCount[c] == clvR)
															{
																useLipid = false;
																break;
															}
...
void fragmentListConstruction(Lipid lipid, Map & paraMap, vector<SpectraHolder> spectraHolder, const SpectrumListPtr& SL)
{
	...
	for (size_t i=0; i<decoy.precursors.size(); i++)
	{
		...
		//cout << endl;
		//cout << decoy.precursors[i].precursorMass << endl << endl;
		//for (size_t j=0; j<masses.size(); j++)
		//{
		//	cout << masses[j] << endl;
		//}
	...
	/*
	cout << "name             " << lipid.name << endl;
	cout << "decoy            " << lipid.decoy << endl; /*
	cout << "totalMass        " << lipid.totalMass << endl;
	cout << "backbone         " << lipid.backbone << endl;
	cout << "backboneformula  " << lipid.backboneFormula << endl;
	cout << "backboneMass     " << lipid.backboneMass << endl;
	cout << "headGroup        " << lipid.headGroup << endl;
	cout << "headGroupFormula "	<< lipid.headGroupFormula << endl;
	cout << "headGroupMass    " << lipid.headGroupMass << endl;
	cout << "FA1              " << lipid.FA1 << endl;
	cout << "FA1Mass          " << lipid.FA1Mass << endl;
	cout << "FA1DoubleBonds   " << lipid.FA1DoubleBonds << endl;
	cout << "SN1bond          " << lipid.SN1bond << endl;
	cout << "FA2              " << lipid.FA2 << endl;
	cout << "FA2Mass          " << lipid.FA2Mass << endl;
	cout << "FA2DoubleBonds   " << lipid.FA2DoubleBonds << endl;
	cout << "SN2bond          " << lipid.SN2bond << endl;
	cout << "FA3              " << lipid.FA3 << endl;
	cout << "FA3Mass          " << lipid.FA3Mass << endl;
	cout << "FA3DoubleBonds   " << lipid.FA3DoubleBonds << endl;
	cout << "SN3bond          " << lipid.SN3bond << endl;
	cout << "FA4              " << lipid.FA4 << endl;
	cout << "FA4Mass          " << lipid.FA4Mass << endl;
	cout << "FA4DoubleBonds   " << lipid.FA4DoubleBonds << endl;
	cout << "SN4bond          " << lipid.SN4bond << endl;
	cout << endl;

	for (size_t k=0; k<lipid.precursors.size(); k++)
	{
		cout << "	description       " << lipid.precursors[k].description << endl;
		cout << "	adduct            " << lipid.precursors[k].adduct << endl;
		cout << "	ESI               " << lipid.precursors[k].ESI << endl;
		cout << "	precursorMass     " << lipid.precursors[k].precursorMass << endl;
		cout << endl;

		for (size_t p=0; p<lipid.precursors[k].fragments.size(); p++)
		{
			cout << "		fragType         " << lipid.precursors[k].fragments[p].fragType << endl;
			cout << "		fragDescription  " << lipid.precursors[k].fragments[p].fragDescription << endl;
			cout << "		fragmentMass     " << lipid.precursors[k].fragments[p].fragmentMass << endl;
			cout << endl;
		}
	}
	*/
	/*
	cout << "name             " << decoy.name << endl;
	cout << "decoy            " << decoy.decoy << endl; /*
	cout << "totalMass        " << decoy.totalMass << endl;
	cout << "backbone         " << decoy.backbone << endl;
	cout << "backboneformula  " << decoy.backboneFormula << endl;
	cout << "backboneMass     " << decoy.backboneMass << endl;
	cout << "headGroup        " << decoy.headGroup << endl;
	cout << "headGroupFormula "	<< decoy.headGroupFormula << endl;
	cout << "headGroupMass    " << decoy.headGroupMass << endl;
	cout << "FA1              " << decoy.FA1 << endl;
	cout << "FA1Mass          " << decoy.FA1Mass << endl;
	cout << "FA1DoubleBonds   " << decoy.FA1DoubleBonds << endl;
	cout << "SN1bond          " << decoy.SN1bond << endl;
	cout << "FA2              " << decoy.FA2 << endl;
	cout << "FA2Mass          " << decoy.FA2Mass << endl;
	cout << "FA2DoubleBonds   " << decoy.FA2DoubleBonds << endl;
	cout << "SN2bond          " << decoy.SN2bond << endl;
	cout << "FA3              " << decoy.FA3 << endl;
	cout << "FA3Mass          " << decoy.FA3Mass << endl;
	cout << "FA3DoubleBonds   " << decoy.FA3DoubleBonds << endl;
	cout << "SN3bond          " << decoy.SN3bond << endl;
	cout << "FA4              " << decoy.FA4 << endl;
	cout << "FA4Mass          " << decoy.FA4Mass << endl;
	cout << "FA4DoubleBonds   " << decoy.FA4DoubleBonds << endl;
	cout << "SN4bond          " << decoy.SN4bond << endl;
	cout << endl;
	
	for (size_t k=0; k<decoy.precursors.size(); k++)
	{
		//cout << "	description       " << decoy.precursors[k].description << endl;
		//cout << "	adduct            " << decoy.precursors[k].adduct << endl;
		//cout << "	ESI               " << decoy.precursors[k].ESI << endl;
		//cout << "	precursorMass     " << decoy.precursors[k].precursorMass << endl;
		cout << endl;
		
		for (size_t p=0; p<decoy.precursors[k].fragments.size(); p++)
		{
			cout << "		fragType         " << decoy.precursors[k].fragments[p].fragType << endl;
			cout << "		fragDescription  " << decoy.precursors[k].fragments[p].fragDescription << endl;
			cout << "		fragmentMass     " << decoy.precursors[k].fragments[p].fragmentMass << endl;
			cout << endl;
		}
	}
	*/
	...
	//scoring(decoy, paraMap, spectraHolder, SL);
...
void scoring(Lipid lipid, Map & paraMap, vector<SpectraHolder> spectraHolder, const SpectrumListPtr& SL)
{
	...
	cout << "name             " << lipid.name << endl;
	cout << "totalMass        " << lipid.totalMass << endl;
	cout << "lipidFormula     " << lipid.lipidFormula << endl;
	cout << "backbone         " << lipid.backbone << endl;
	cout << "backboneformula  " << lipid.backboneFormula << endl;
	cout << "backboneMass     " << lipid.backboneMass << endl;
	cout << "headGroup        " << lipid.headGroup << endl;
	...
	for (size_t k=0; k<lipid.precursors.size(); k++)
	{
		cout << "	description       " << lipid.precursors[k].description << endl;
		cout << "	adduct            " << lipid.precursors[k].adduct << endl;
		cout << "	adduct type       " << lipid.precursors[k].adductType << endl;
		cout << "	ESI               " << lipid.precursors[k].ESI << endl;
		cout << "	precursorMass     " << lipid.precursors[k].precursorMass << endl;
		cout << endl;
	...
	*/
	...
	//for (size_t i=0; i<spectraHolder.size(); i++)
	//{
		//cout << spectraHolder[i].spectraIndex << "  " << spectraHolder[i].preMassN << "  " << spectraHolder[i].preMassP << endl;
	//}
	...
	for (size_t i=0; i<lipid.precursors.size(); i++)
	{
		...
		//int searchIndex = binarySearch(spectraHolder, test, 0, spectraHolder.size()-1);
		int searchIndex=0;
		...
		//cout << test << "  " << spectraHolder[n].preMassN << "  " << spectraHolder[n].preMassP << endl; 
		...
		{	
			...
			/*
			// print matched peak list
			for (size_t m=0; m<match.size(); m++)
			{
				cout << "  m/z: " << match[m].mz << "  match: " << match[m].match << "  intensity: " << match[m].intensity  
					<< "  fragment: " << match[m].fragmentT << "   " << match[m].fragmentD << endl; 
			}
			cout << endl;
			*/
			...
			match.resize(indexB);
			/*
			// print spectrum peak list
			int asdf = 0;
			for (vector<MZIntensityPair>::const_iterator it = pairs.begin(), end = pairs.end(); it!=end; ++it)
			{
				asdf++;
				cout << asdf <<  "  m/z: " << it->mz << "   intensity: " << it->intensity << endl;
			}
			cout << endl;
			...
			*/
			...
			//double crap = log10(peakScore)/x;
			//peakScore = pow(10, crap);
			...
			else
			{
				...
				//totalScore = -2*(log(pow(10,iScore2)));
			}
			//cout << totalScore << "  " << intensitySum << endl;
			...
			/*
			specAll[spectraHolder[n].spectraIndex].push_back(Score());
			specAll[spectraHolder[n].spectraIndex][specAll[spectraHolder[n].spectraIndex].size()-1].totalScore = totalScore;
			specAll[spectraHolder[n].spectraIndex][specAll[spectraHolder[n].spectraIndex].size()-1].intensityScore = iScore2;
			specAll[spectraHolder[n].spectraIndex][specAll[spectraHolder[n].spectraIndex].size()-1].peakScore = peakScore;
			specAll[spectraHolder[n].spectraIndex][specAll[spectraHolder[n].spectraIndex].size()-1].spectrumNumber = spectraHolder[n].spectraIndex;
			specAll[spectraHolder[n].spectraIndex][specAll[spectraHolder[n].spectraIndex].size()-1].spectrumPrecursorMass = mass;
			specAll[spectraHolder[n].spectraIndex][specAll[spectraHolder[n].spectraIndex].size()-1].lipidName = lipid.name;
			specAll[spectraHolder[n].spectraIndex][specAll[spectraHolder[n].spectraIndex].size()-1].lipidMass = lipid.totalMass;
			specAll[spectraHolder[n].spectraIndex][specAll[spectraHolder[n].spectraIndex].size()-1].chemFormula = lipid.lipidFormula;
			specAll[spectraHolder[n].spectraIndex][specAll[spectraHolder[n].spectraIndex].size()-1].precursorDescription = lipid.precursors[i].description;
			specAll[spectraHolder[n].spectraIndex][specAll[spectraHolder[n].spectraIndex].size()-1].precursorMass = lipid.precursors[i].precursorMass;
			specAll[spectraHolder[n].spectraIndex][specAll[spectraHolder[n].spectraIndex].size()-1].match = match;
			specAll[spectraHolder[n].spectraIndex][specAll[spectraHolder[n].spectraIndex].size()-1].ms2list = ms2list;
			specAll[spectraHolder[n].spectraIndex][specAll[spectraHolder[n].spectraIndex].size()-1].retTime = spectraHolder[n].retentionTime;
			specAll[spectraHolder[n].spectraIndex][specAll[spectraHolder[n].spectraIndex].size()-1].mod = lipid.precursors[i].adduct;

			if (lipid.precursors[i].ESI == "pos")
			{
				specAll[spectraHolder[n].spectraIndex][specAll[spectraHolder[n].spectraIndex].size()-1].charge = "+1";
			}
			if ((lipid.precursors[i].ESI == "neg") && (lipid.precursors[i].description != "deprotonatedX2"))
			{
				specAll[spectraHolder[n].spectraIndex][specAll[spectraHolder[n].spectraIndex].size()-1].charge = "-1";
			...
			if ((lipid.precursors[i].ESI == "neg") && (lipid.precursors[i].description == "deprotonatedX2"))
			{
				specAll[spectraHolder[n].spectraIndex][specAll[spectraHolder[n].spectraIndex].size()-1].charge = "-2";
			}


			sort(specAll[spectraHolder[n].spectraIndex].begin(), specAll[spectraHolder[n].spectraIndex].end(), sortByName);
			sort(specAll[spectraHolder[n].spectraIndex].begin(), specAll[spectraHolder[n].spectraIndex].end(), sortByTotalScore);
			...
			{
				if ((spec[spectraHolder[n].spectraIndex][m].totalScore != tScore) || ((spec[spectraHolder[n].spectraIndex][m].totalScore == tScore) && (spec[spectraHolder[n].spectraIndex][m].intensityScore != tieBreaker)))
				{
					tScore = spec[spectraHolder[n].spectraIndex][m].totalScore;
					tieBreaker = spec[spectraHolder[n].spectraIndex][m].intensityScore;
					change++;
					tempoIndex++;
				}
				else
				...
				//cout << setprecision(20) << tScore << "   " << tempoIndex << "   " << change << endl;
				if (change == 4)
				{
					spec[spectraHolder[n].spectraIndex].resize(tempoIndex);
					break;
				}
			}
			*/
			...
			for (size_t m=0; m<spec[spectraHolder[n].spectraIndex].size(); m++)
			{
				if (lipid.precursors[i].ESI == "pos")
				{
					...
					if (spec[spectraHolder[n].spectraIndex][m].lipidName.substr(0, 2) == "PE" && pe == 0 && (spec[spectraHolder[n].spectraIndex][m].mod == "+H" || spec[spectraHolder[n].spectraIndex][m].mod == "+NH4"))
					{
						specTemp.push_back(spec[spectraHolder[n].spectraIndex][m]);
						pe = 1;
					}
					if (spec[spectraHolder[n].spectraIndex][m].lipidName.substr(0, 2) == "PE" && mpe == 0 && spec[spectraHolder[n].spectraIndex][m].mod != "+H" && spec[spectraHolder[n].spectraIndex][m].mod != "+NH4")
					{
						specTemp.push_back(spec[spectraHolder[n].spectraIndex][m]);
						mpe = 1;
					}
				...
				if (lipid.precursors[i].ESI == "neg")
				{
					...
					if (spec[spectraHolder[n].spectraIndex][m].lipidName.substr(0, 2) == "PE" && pe == 0)
					{
						specTemp.push_back(spec[spectraHolder[n].spectraIndex][m]);
						pe = 1;
					}
			...
			if (lipid.precursors[i].ESI == "pos")
			{
				...
				if (lipid.name.substr(0, 2) == "PE" && (lipid.precursors[i].adduct == "+H" || lipid.precursors[i].adduct == "+NH4"))
				{
				...
				if (lipid.name.substr(0, 2) == "CL" && (lipid.precursors[i].adduct == "-2H + 3Na" || lipid.precursors[i].adduct == "-2H + 3Li" || lipid.precursors[i].adduct == "-2H + 3K"))
				{
					if (totalScore > secondary[spectraHolder[n].spectraIndex].M3CL[0])
					{
						secondary[spectraHolder[n].spectraIndex].M3CL[1] = secondary[spectraHolder[n].spectraIndex].M3CL[0];
						secondary[spectraHolder[n].spectraIndex].M3CL[0] = totalScore;
					}
					if (totalScore < secondary[spectraHolder[n].spectraIndex].M3CL[0] && totalScore > secondary[spectraHolder[n].spectraIndex].M3CL[1])
					{
						secondary[spectraHolder[n].spectraIndex].M3CL[1] = totalScore;
					}
				}
				...
				/*
				if (lipid.name.substr(0, 2) == "PE-Cer" && (lipid.precursors[i].adduct == "+H" || lipid.precursors[i].adduct == "+NH4"))
				{
				...
				if (lipid.name.substr(0, 2) == "PI-Cer" && (lipid.precursors[i].adduct == "+H" || lipid.precursors[i].adduct == "+NH4"))
				{
