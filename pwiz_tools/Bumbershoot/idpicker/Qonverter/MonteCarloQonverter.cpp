                                  ...
                                  WeightsByChargeAndBestSpecificity* monteCarloWeightsByChargeAndBestSpecificity)
{
    ...
    BOOST_FOREACH(const PSMIteratorRange& range, psmPartitionedRows)
    {
        ...
        BOOST_LOG_SEV(logSource::get(), MessageSeverity::VerboseInfo) << "The best Monte Carlo weights for PSMs of charge " << range.front().chargeState << " and terminal specificity " << range.front().bestSpecificity << " are (" << *bestSolution << "), producing " << passingPSMs << " ids.";
