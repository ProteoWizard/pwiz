...
namespace IDPicker.DataModel
{
    public class Parser : IDisposable
    {
        ...
        public delegate IDictionary<Analysis, QonverterSettings> QonverterSettingsHandler (IList<Analysis> analyses, out bool cancel);
        #endregion
        ...
        public Dictionary<string, List<string>> XmlFilepathsByIdpDbFilepath { get { return xmlFilepathsByIdpDbFilepath; } } 

        string rootInputDirectory;
        QonverterSettingsHandler qonverterSettingsHandler;
        ...
        Dictionary<string, List<string>> xmlFilepathsByIdpDbFilepath = new Dictionary<string, List<string>>();
        ...
        public Parser (string rootInputDirectory, QonverterSettingsHandler qonverterSettingsHandler, bool mergeDirectories, params string[] xmlFilepaths)
        {
            ...
            this.qonverterSettingsHandler = qonverterSettingsHandler;
            this.xmlFilepathList = new List<string>(xmlFilepaths);

            xmlFilepathsByIdpDbFilepath = determineIdpDbFilepaths(mergeDirectories, xmlFilepaths);
        }

        private static Dictionary<string, List<string>> determineIdpDbFilepaths (bool mergeDirectories, IList<string> xmlFilepaths)
        {
            var xmlFilepathsByIdpDbFilepath = new Dictionary<string, List<string>>();
            var xmlFilepathsByDirectory = new Dictionary<string, IEnumerable<string>>();

            foreach (string xmlFilepath in xmlFilepaths)
            {
                if (!mergeDirectories)
                {
                    string idpDbFilepath = Path.ChangeExtension(xmlFilepath, ".idpDB");
                    if (!xmlFilepathsByIdpDbFilepath.ContainsKey(idpDbFilepath))
                        xmlFilepathsByIdpDbFilepath[idpDbFilepath] = new List<string>();
                    xmlFilepathsByIdpDbFilepath[idpDbFilepath].Add(xmlFilepath);
                    continue;
                }

                string path = Path.GetDirectoryName(xmlFilepath);
                if (!xmlFilepathsByDirectory.ContainsKey(path))
                {
                    xmlFilepathsByDirectory[path] = xmlFilepaths.Where(o => Path.GetDirectoryName(o) == path);
                    xmlFilepathsByIdpDbFilepath[Util.GetCommonFilename(xmlFilepathsByDirectory[path])] = xmlFilepathsByDirectory[path].ToList();
                }
            }
            return xmlFilepathsByIdpDbFilepath;
        }

        Queue<string> idpDbFilepathQueue;
        ...
        public bool Start ()
        {
            ...
            var distinctAnalyses = new List<Analysis>();
            ...
            foreach (string xmlFilepath in xmlFilepathList)
            {
                try
                {
                    using (var sourceXml = new StreamReader(xmlFilepath))
                    {
                        FileType fileType;
                        long fileSize = 0;
                        var sourceNames = sourceNamesByXmlFilepath[xmlFilepath] = new List<string>();
                        var fileAnalyses = new List<Analysis>();
                        IdentifyXml(sourceXml, out fileType, out fileSize, out proteinDatabasePath, sourceNames, fileAnalyses);

                        foreach (Analysis fileAnalysis in fileAnalyses)
                        {
                            // an analysis is unique if its name is unique and its parameter set has some
                            // difference with other analyses
                            if (distinctAnalyses.Count(a => a.Name == fileAnalysis.Name &&
                                                            a.Parameters.ExclusiveOr(fileAnalysis.Parameters).Count == 0) == 0)
                            {
                                fileAnalysis.Id = distinctAnalyses.Count + 1;
                                distinctAnalyses.Add(fileAnalysis);
                            }
                        }
                        ...
                        foreach (string sourceName in sourceNames)
                        {
                            try
                            {
                                if(rawDataFilepathByName.ContainsKey(sourceName))
                                    continue;
            ...
            bool cancel;
            qonverterSettingsByAnalysis = qonverterSettingsHandler(distinctAnalyses, out cancel);
            ...
            idpDbFilepathQueue = new Queue<string>(xmlFilepathsByIdpDbFilepath.Keys);
        ...
        {
            foreach(var idpDbFilepath in xmlFilepathsByIdpDbFilepath.Keys)
                foreach (var xmlFilepath in xmlFilepathsByIdpDbFilepath[idpDbFilepath])
                {
                    try
                    {
                        EmbedSourceMzML(idpDbFilepath);
            ...
            using (XmlReader reader = XmlTextReader.Create(xmlStream, settings))
            {
                ...
                if (foundXMLTag)
                {
                    ...
                    while (!foundDatabase && reader.Read() && numTagsRead < 20)
                    {
                        ...
                        else if (reader.Name == "search_summary")
                        {
                        ...
                        while (reader.Read() && sourceNames.Count < expectedSourceCount)
                        {
                            switch (reader.NodeType)
                            {
                                case XmlNodeType.Element:
                                    ...
                                    {
                                        sourceName = getAttribute(reader, "name");
                                    }
                                    break;
                    ...
                    else if (fileType == FileType.PepXML)
                    {
                        while (reader.Read() && !(reader.NodeType == XmlNodeType.EndElement && reader.Name == "search_summary"))
                        {
        ...
        void doReadXml ()
        {
            while (true)
            {
                ...
                lock (queueMutex)
                {
                    if (idpDbFilepathQueue.Count == 0)
                        return;
                    idpDbFilepath = idpDbFilepathQueue.Dequeue();
                }

                File.Delete(idpDbFilepath);
                //var conn = SessionFactoryFactory.CreateFile(xmlFilepath + ".idpDB");

                foreach (string xmlFilepath in xmlFilepathsByIdpDbFilepath[idpDbFilepath])
                {
                    ...
                    IdentifyXml(sourceXml, out fileType, out fileSize, out proteinDatabasePath, sourceNames, analyses);
                    sourceXml.Close();
                    ...
                    else
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            switch (reader.NodeType)
                            {
                                ...
                                case XmlNodeType.EndElement:
                                    ...
                                    {
                                        var distinctAnalyses = qonverterSettingsByAnalysis.Keys;
                        ...
                        var qonverterSettings = qonverterSettingsByAnalysis[curAnalysis];
                        qonverterSettings.Analysis = curAnalysis;
                        bulkInserter.Add(qonverterSettings);
...
";
                    ...
                    if (Path.ChangeExtension(xmlFilepath, ".idpDB") != idpDbFilepath)
                    {
                        (conn as SQLiteConnection).SaveToDisk(Path.ChangeExtension(xmlFilepath, ".idpDB"));
                        var merger = new Merger(idpDbFilepath, conn as SQLiteConnection);
                        merger.Start();
                    }
                    else
                        (conn as SQLiteConnection).SaveToDisk(Path.ChangeExtension(xmlFilepath, ".idpDB"));
        ...
        string locateProteinDatabase (string proteinDatabasePath)
        {
            ...
            catch
            {
                try
                {
                    return Util.FindDatabaseInSearchPath(proteinDatabasePath, Properties.Settings.Default.LastProteinDatabaseDirectory);
                }
                catch
                {
                    if (DatabaseNotFound != null)
                    {
                        ...
                        {
                            Properties.Settings.Default.LastProteinDatabaseDirectory = Path.GetDirectoryName(eventArgs.DatabasePath);
                ...
                while (true)
                {
                    ...
                    lock (queueMutex)
                    {
                        if (idpDbFilepathQueue.Count == 0)
                            return;
                        idpDbFilepath = idpDbFilepathQueue.Dequeue();
                        parsedBytes += xmlFilepathsByIdpDbFilepath[idpDbFilepath].Sum(o => new FileInfo(o).Length);
                    }
                    ...
                    if(firstBatch)
                        try
                        {
                            conn.ExecuteNonQuery("DELETE FROM Protein;");
                            conn.ExecuteNonQuery("DELETE FROM ProteinMetadata;");
                            conn.ExecuteNonQuery("DELETE FROM ProteinData;");
                            conn.ExecuteNonQuery("DELETE FROM PeptideInstance");
                        }
                        catch { }
                    ...
                    while (true)
                    {
                        ...
                        foreach (var queryRow in conn.ExecuteQuery("SELECT Sequence, Id FROM PeptideSequences LIMIT " + offset + "," + LIMIT))
                            peptideBySequence[(string) queryRow[0]] = new Peptide {Id = (long) queryRow[1]};

                        if (peptideBySequence.Count == 0)
                            break;
                        ...
                        {
                            var dbProtein = new Protein(proteomePair.Key.description,
                                                        System.Text.RegularExpressions.Regex.Replace(proteomePair.Key.sequence, "[^A-IK-NP-Z]", ""))
                                                {
                                                    Id = ++proteinLastId,
                            ...
                            {
                                if (!proteinSet.Contains(dbProtein.Accession))
                                {
