    ...
    public partial class BaseFileDialogNE : CommonFormEx
    {
        ...
        public BaseFileDialogNE(string[] sourceTypes, IList<RemoteAccount> remoteAccounts, IList<string> specificDataSourceFilter = null, bool isRemote = false)
        {
            ...
            TreeNode remoteNode = tv.Nodes.Add(@"Remote",
                CommonFileDialogResources.OpenDataSourceDialog_OpenDataSourceDialog_Remote_Accounts, (int) ImageIndex.MyNetworkPlaces,
            ...
            if (!isRemote)
            {
                TreeNode desktopNode = tv.Nodes.Add(@"Desktop",
                    CommonFileDialogResources.OpenDataSourceDialog_OpenDataSourceDialog_Desktop, (int)ImageIndex.Desktop, (int)ImageIndex.Desktop);
                ...
                TreeNode lookInNode = desktopNode.Nodes.Add(@"My Documents",
                    CommonFileDialogResources.OpenDataSourceDialog_OpenDataSourceDialog_My_Documents, (int)ImageIndex.MyDocuments, (int)ImageIndex.MyDocuments);
                ...
                TreeNode myComputerNode = desktopNode.Nodes.Add(@"My Computer",
                    CommonFileDialogResources.OpenDataSourceDialog_OpenDataSourceDialog_My_Computer, (int)ImageIndex.MyComputer, (int)ImageIndex.MyComputer);
            ...
            if (url is RemoteUrl remoteUrl && !remoteUrl.Equals(RemoteUrl.EMPTY)) // if RemoteUrl
            {
                ...
                else
                {
                    ...
                    {   // If invalid account
                        ...
                        // Show error message
                        var message = CommonTextUtil.LineSeparate(
                            string.Format(CommonFileDialogResources.BaseFileDialogNE_Authentication_exception_message,
                                remoteAccount.AccountAlias, errorType.ToUserMessage()));
                        if (!string.IsNullOrEmpty(msg))
                        {
                            message = CommonTextUtil.LineSeparate(message,
        ...
        private void populateListViewFromDirectory(MsDataFileUri directory)
        {
            ...
            if (null == directory || directory is MsDataFilePath && string.IsNullOrEmpty(((MsDataFilePath) directory).FilePath))
            {
                foreach (DriveInfo driveInfo in DriveInfo.GetDrives())
                {
                    ...
                    try
                    {
                        switch (driveInfo.DriveType)
                        {
                            case DriveType.Fixed:
                                ...
                                label = CommonFileDialogResources.OpenDataSourceDialog_populateListViewFromDirectory_Local_Drive;
                                if (driveInfo.VolumeLabel.Length > 0)
                                    label = driveInfo.VolumeLabel;
                                break;
                            case DriveType.CDRom:
                                imageIndex = ImageIndex.OpticalDrive;
                                label = CommonFileDialogResources.OpenDataSourceDialog_populateListViewFromDirectory_Optical_Drive;
                                if (driveInfo.IsReady && driveInfo.VolumeLabel.Length > 0)
                                    label = driveInfo.VolumeLabel;
                                break;
                            case DriveType.Removable:
                                imageIndex = ImageIndex.OpticalDrive;
                                label = CommonFileDialogResources.OpenDataSourceDialog_populateListViewFromDirectory_Removable_Drive;
                                if (driveInfo.IsReady && driveInfo.VolumeLabel.Length > 0)
                                    label = driveInfo.VolumeLabel;
                                break;
                            case DriveType.Network:
                                label = CommonFileDialogResources.OpenDataSourceDialog_populateListViewFromDirectory_Network_Share;
                                break;
                        }
                    ...
                    {
                        label += string.Format(@" ({0})", CommonFileDialogResources.OpenDataSourceDialog_populateListViewFromDirectory_access_failure);
                    }
            ...
            else if (directory is RemoteUrl)
            {
                ...
                if (string.IsNullOrEmpty(remoteUrl.ServerUrl))
                {
                    ...
                    {
                        name = CommonFileDialogResources.BaseFileDialogNE_populateListViewFromDirectory_Add_Edit_Account___,
                ...
                else
                {
                    ...
                    if (null != exception)
                    {
                        if (ShowRetryDialog(exception.Message, CommonFileDialogResources.OpenDataSourceDialog_populateListViewFromDirectory_Retry) != DialogResult.Cancel)
                        {
                ...
                foreach (DriveInfo driveInfo in DriveInfo.GetDrives())
                {
                    ...
                    try
                    {
                        switch (driveInfo.DriveType)
                        {
                            case DriveType.Fixed:
                                imageIndex = ImageIndex.LocalDrive;
                                label = CommonFileDialogResources.OpenDataSourceDialog_populateComboBoxFromDirectory_Local_Drive;
                                if (driveInfo.VolumeLabel.Length > 0)
                                    label = driveInfo.VolumeLabel;
                                break;
                            case DriveType.CDRom:
                                imageIndex = ImageIndex.OpticalDrive;
                                label = CommonFileDialogResources
                    ...
                    catch (Exception)
                    {
                        label += string.Format(@" ({0})",
                            CommonFileDialogResources.OpenDataSourceDialog_populateComboBoxFromDirectory_access_failure);
                    }
        ...
        private bool EditAccount()
        {
            ...
            {
                var editedAccounts = OnEditAccounts(this);
                if (editedAccounts != null)
                {
                    SetRemoteAccounts(editedAccounts);
                }
...
//        }
            ...
            private string DateModifiedLabel
            {
                get
                {
                    return dateModified.HasValue
                        ? CommonTextUtil.SpaceSeparate(dateModified.Value.ToShortDateString(),
        ...
        private void EnsureRemoteAccount()
        {
            ...
            var newAccount = OnCreateNewAccount(this);
            if (null != newAccount)
            {
        ...
        /// <summary>
        /// Whether this dialog should check method development support when there is exactly one remote account.
        /// Override in subclasses like WatersConnectMethodFileDialog.
        /// </summary>
        protected virtual bool ShouldCheckMethodDevelopmentSupport => false;

        /// <summary>
        /// Returns a fallback dialog owner when the form is not yet visible.
        /// Override in application-specific subclasses to return the main window.
        /// </summary>
        protected virtual IWin32Window GetFallbackDialogOwner()
        {
            return null;
        }
        ...
        /// Override in application-specific subclasses to use custom dialog implementations.
        /// </summary>
        protected virtual void ShowErrorMessage(IWin32Window owner, string message)
        {
        ...
        /// Override in application-specific subclasses to use custom dialog implementations.
        /// </summary>
        protected virtual void ShowErrorWithException(IWin32Window owner, string message, Exception exception)
        {
            MessageBox.Show(owner, message, Text, MessageBoxButtons.OK, MessageBoxIcon.Error);
        }

        /// <summary>
        /// Shows a retry/cancel dialog and returns the DialogResult.
        /// Override in application-specific subclasses to use custom dialog implementations.
        /// </summary>
        protected virtual DialogResult ShowRetryDialog(string message, string retryButtonText)
        {
            return MessageBox.Show(this, message, Text, MessageBoxButtons.RetryCancel, MessageBoxIcon.Warning);
        }

        /// <summary>
        /// Called when the user wants to edit remote accounts.
        /// Must be overridden in application-specific subclasses to provide account editing UI.
        /// Returns the updated list of accounts, or null if the user cancelled.
        /// </summary>
        protected virtual IList<RemoteAccount> OnEditAccounts(IWin32Window owner)
        {
            return null;
        }

        /// <summary>
        /// Called when a new remote account needs to be created.
        /// Must be overridden in application-specific subclasses to provide account creation UI.
        /// Returns the new account, or null if the user cancelled.
        /// </summary>
        protected virtual RemoteAccount OnCreateNewAccount(IWin32Window owner)
        {
            return null;
        }
