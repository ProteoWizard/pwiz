...
namespace pwiz.Common.Chemistry.SeparateArrays
{
    /// <summary>
    ...
    public class MassDistribution : IReadOnlyList<KeyValuePair<double, double>>
    {
        ...
        {
            var massAbundanceArray = PartialAdd(0, Count, rhs);
            return ApplyBinning(massAbundanceArray);
        }

        private MassFrequencies PartialAdd(int start, int end, MassDistribution rhs)
        {
            if (end == start + 1)
            {
                var myMass = _masses[start];
                var myFrequency = _frequencies[start];
                var newMasses = new double[rhs.Count];
                var newAbundances = new double[rhs.Count];
                for (int i = 0; i < rhs.Count; i++)
                {
                    newMasses[i] = myMass + rhs._masses[i];
                    newAbundances[i] = myFrequency * rhs._frequencies[i];
                }

                return new MassFrequencies(newMasses, newAbundances);
            }
            var mid = (start + end) / 2;
        ...
        private static MassFrequencies Merge(MassFrequencies list1, MassFrequencies list2)
        {
            int resultCount = 0;
            var resultMasses = new double[list1.Count + list2.Count];
            var resultFrequencies = new double[list1.Count + list2.Count];
            int index1 = 0;
            int index2 = 0;
            while (true)
            {
                ...
                if (index1 < list1.Count)
                {
                    ...
                    {
                        compare = list1.Masses[index1].CompareTo(list2.Masses[index2]);
                    }
                    else
                    {
                        compare = -1;
                ...
                if (compare < 0)
                {
                    resultMasses[resultCount] = list1.Masses[index1];
                    resultFrequencies[resultCount] = list1.Frequencies[index1];
                }
                else if (compare > 0)
                {
                    resultMasses[resultCount] = list2.Masses[index2];
                    resultFrequencies[resultCount] = list2.Frequencies[index2];
                }
                else
                {
                    resultMasses[resultCount] = list1.Masses[index1];
                    resultFrequencies[resultCount] = list1.Frequencies[index1] + list2.Frequencies[index2];
                }
                resultCount++;
                if (compare <= 0)
                {
            ...
            return new MassFrequencies(resultMasses, resultFrequencies, resultCount);
        }
        ...
        private MassDistribution ApplyBinning(MassFrequencies massFrequencies)
        {
            var count = massFrequencies.Count;
            ...
            for (int i = 0; i < count; i++)
            {
                var frequency = massFrequencies.Frequencies[i];
                if (frequency.Equals(0.0))
                    continue;

                var mass = massFrequencies.Masses[i];
        ...
        {
            var list = values.OrderBy(value => value.Key).ToList();
            var massAbundanceArray = new MassFrequencies(list.Select(kvp => kvp.Key).ToArray(),
                list.Select(kvp => kvp.Value).ToArray());
            return new MassDistribution(massResolution, minimumAbundance)
                .ApplyBinning(massAbundanceArray);
        }

        private class MassFrequencies
        {
            public MassFrequencies(double[] masses, double[] frequencies) : this(masses, frequencies, masses.Length)
            {
                
            }
            public MassFrequencies(double[] masses, double[] frequencies, int count)
            {
                Masses = masses;
