...
namespace pwiz.Common.Chemistry.Alt
{
    /// <summary>
    ...
    public class MassDistribution : IReadOnlyList<KeyValuePair<double, double>>
    {
        ...
        private List<Tuple<double, double>> PartialAdd(int start, int end, MassDistribution rhs)
        {
            if (end == start + 1)
            {
                var myMass = _masses[start];
                var myFrequency = _frequencies[start];
                var list = new List<Tuple<double, double>>(rhs.Count);
                for (int i = 0; i < rhs.Count; i++)
                {
                    list.Add(Tuple.Create(myMass + rhs._masses[i],
                        myFrequency * rhs._frequencies[i]));
                }
                return list;
            }
            var mid = (start + end) / 2;
        ...
        private static List<Tuple<double, double>> Merge(List<Tuple<double, double>> list1,
            List<Tuple<double, double>> list2)
        {
            var resultArray = new List<Tuple<double, double>>(list1.Count + list2.Count);
            int index1 = 0;
            int index2 = 0;
            while (true)
            {
                ...
                if (index1 < list1.Count)
                {
                    ...
                    {
                        compare = list1[index1].Item1.CompareTo(list2[index2].Item1);
                    }
                    else
                    {
                        compare = -1;
                ...
                if (compare < 0)
                {
                    resultArray.Add(list1[index1]);
                }
                else if (compare > 0)
                {
                    resultArray.Add(list2[index2]);
                }
                else
                {
                    resultArray.Add(Tuple.Create(list1[index1].Item1, list1[index1].Item2 + list2[index2].Item2));
                }
                if (compare <= 0)
                {
                    index1++;
        ...
        private MassDistribution ApplyBinning(List<Tuple<double, double>> array)
        {
        ...
        {
            var array = values.Select(kvp => Tuple.Create(kvp.Key, kvp.Value)).OrderBy(tuple => tuple.Item1).ToList();
            return new MassDistribution(massResolution, minimumAbundance)
                .ApplyBinning(array);
        }
    }
