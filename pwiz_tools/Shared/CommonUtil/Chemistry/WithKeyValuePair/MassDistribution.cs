...
namespace pwiz.Common.Chemistry.WithKeyValuePair
{
    /// <summary>
    ...
    public class MassDistribution : IReadOnlyList<KeyValuePair<double, double>>, IMassDistribution<MassDistribution>
    {
        ...
        private List<KeyValuePair<double,double>> PartialAdd(int start, int end, MassDistribution rhs)
        {
            if (end == start + 1)
            {
                ...
                int count = rhs.Count;
                var list = new List<KeyValuePair<double,double>>(count);
                for (int i = 0; i < count; i++)
                {
                    list.Add(new KeyValuePair<double,double>(myMass + rhs._masses[i],
        ...
        private static List<KeyValuePair<double,double>> Merge(List<KeyValuePair<double,double>> list1,
            List<KeyValuePair<double,double>> list2)
        {
            int count1 = list1.Count;
            int count2 = list2.Count;
            var resultArray = new List<KeyValuePair<double,double>>(count1 + count2);
            int index1 = 0;
            int index2 = 0;
            while (true)
            {
                ...
                if (index1 < count1)
                {
                    if (index2 < count2)
                    {
                        compare = list1[index1].Key.CompareTo(list2[index2].Key);
                    }
                    else
                    {
                        compare = -1;
                ...
                {
                    resultArray.Add(new KeyValuePair<double,double>(list1[index1].Key, list1[index1].Value + list2[index2].Value));
                }
                if (compare <= 0)
                {
                    index1++;
        ...
        private MassDistribution ApplyBinning(List<KeyValuePair<double,double>> array)
        {
        ...
        {
            var array = values.Select(kvp => new KeyValuePair<double,double>(kvp.Key, kvp.Value)).OrderBy(tuple => tuple.Key).ToList();
            return new MassDistribution(massResolution, minimumAbundance)
                .ApplyBinning(array);
        }
    }
