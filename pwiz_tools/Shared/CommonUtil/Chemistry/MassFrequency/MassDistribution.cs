...
namespace pwiz.Common.Chemistry.MassFrequency
{
    /// <summary>
    ...
    public class MassDistribution : IReadOnlyList<KeyValuePair<double, double>>
    {
        ...
        private Tuple<MassFrequency[], int> PartialAdd(int start, int end, MassDistribution rhs)
        {
            if (end == start + 1)
            {
                var myMass = _masses[start];
                var myFrequency = _frequencies[start];
                var array = new MassFrequency[rhs.Count];
                for (int i = 0; i < rhs.Count; i++)
                {
                    array[i] = new MassFrequency(myMass + rhs._masses[i], myFrequency * rhs._frequencies[i]);
                }
        ...
        private static Tuple<MassFrequency[], int> Merge(Tuple<MassFrequency[], int> list1,
            Tuple<MassFrequency[], int> list2)
        {
            int resultCount = 0;
            var resultArray = new MassFrequency[list1.Item2 + list2.Item2];
            int index1 = 0;
            int index2 = 0;
            while (true)
            {
                ...
                if (index1 < list1.Item2)
                {
                    ...
                    {
                        compare = list1.Item1[index1].Mass.CompareTo(list2.Item1[index2].Mass);
                    }
                    else
                    {
                        compare = -1;
                ...
                {
                    resultArray[resultCount] = new MassFrequency(list1.Item1[index1].Mass, list1.Item1[index1].Frequency+ list2.Item1[index2].Frequency);
                }
                resultCount++;
                if (compare <= 0)
                {
        ...
        private MassDistribution ApplyBinning(MassFrequency[] array, int count)
        {
        ...
        {
            var array = values.OrderBy(value=>value.Key).Select(value=>new MassFrequency(value.Key, value.Value)).ToArray();
            return new MassDistribution(massResolution, minimumAbundance)
                .ApplyBinning(array, array.Length);
        }

        private struct MassFrequency
        {
            public readonly double Mass;
            public readonly double Frequency;
            public MassFrequency(double mass, double frequency)
            {
                Mass = mass;
                Frequency = frequency;
            }
