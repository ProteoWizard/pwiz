...
namespace pwiz.Common.Chemistry.WithValueTuple
{
    /// <summary>
    ...
    public class MassDistribution : IReadOnlyList<KeyValuePair<double, double>>
    {
        ...
        private List<ValueTuple<double, double>> PartialAdd(int start, int end, MassDistribution rhs)
        {
            if (end == start + 1)
            {
                ...
                int count = rhs.Count;
                var list = new List<ValueTuple<double, double>>(count);
                for (int i = 0; i < count; i++)
                {
                    list.Add(ValueTuple.Create(myMass + rhs._masses[i],
        ...
        private static List<ValueTuple<double, double>> Merge(List<ValueTuple<double, double>> list1,
            List<ValueTuple<double, double>> list2)
        {
            int count1 = list1.Count;
            int count2 = list2.Count;
            var resultArray = new List<ValueTuple<double, double>>(count1 + count2);
            int index1 = 0;
            int index2 = 0;
            while (true)
            {
                ...
                {
                    resultArray.Add(ValueTuple.Create(list1[index1].Item1, list1[index1].Item2 + list2[index2].Item2));
                }
                if (compare <= 0)
                {
                    index1++;
        ...
        private MassDistribution ApplyBinning(List<ValueTuple<double, double>> array)
        {
        ...
        {
            var array = values.Select(kvp => ValueTuple.Create(kvp.Key, kvp.Value)).OrderBy(tuple => tuple.Item1).ToList();
            return new MassDistribution(massResolution, minimumAbundance)
                .ApplyBinning(array);
        }
    }
