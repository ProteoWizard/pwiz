...
{
    public interface IRemoteAccountStorage
    {
        /// <summary>
        /// Provides the RemoteApi code with the user accounts stored by the calling application.
        /// </summary>
    ...
    public abstract class RemoteUrl : MsDataFileUri
    {
        ...
        {
            if (RemoteAccountStorage == null)
                throw new InvalidOperationException(@"RemoteAccountStorage must be set to provide remote accounts"); // obvious bug, shouldn't need l10n

            var accounts = RemoteAccountStorage.GetRemoteAccounts();
