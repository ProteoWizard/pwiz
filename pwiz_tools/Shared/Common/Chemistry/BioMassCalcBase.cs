...
// ReSharper restore InconsistentNaming
        ...
        /// and default atom percent enrichment for Skyline's IsotopeEnrichmentItem class.
        ...
        {
            // The only two user-facing strings can be localized here in the constructor
            // A bit clunky but there isn't a well maintained L10N resource set for this namespace
            if (!string.IsNullOrEmpty(msg_The_expression__0__is_not_a_valid_chemical_formula_))
            {
                The_expression__0__is_not_a_valid_chemical_formula = msg_The_expression__0__is_not_a_valid_chemical_formula_;
            }

            if (!string.IsNullOrEmpty(msg_Supported_chemical_symbols_include_))
            {
                Supported_chemical_symbols_include = msg_Supported_chemical_symbols_include_;
            }
            MassType = type;
        ...
        {
            return desc.Any(kvp => DICT_HEAVYSYMBOL_TO_MONOSYMBOL.ContainsKey(kvp.Key)); // Look for Cl', O", D, T etc
        }
        ...
        {
            var result = atomCounts.ToDictionary(x => x.Key, x => x.Value);
            ...
            foreach (var kvp in result.ToArray())
            {
                ...
                if (DICT_HEAVYSYMBOL_TO_MONOSYMBOL.TryGetValue(kvp.Key, out var unlabeled))
                {
                    result.TryGetValue(unlabeled, out var count); // Get current count of unlabeled version, if any
        ...
        public MoleculeMassOffset FindFormulaIntersection(IList<MoleculeMassOffset> formulas)
        {
            ...
            return formulas[0].ChangeFormula(common);
        }
