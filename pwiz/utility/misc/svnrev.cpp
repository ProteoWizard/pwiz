...
int go(const Config& config)
{
    ...
    time_t maxLastWriteTime, existingHeaderLastWriteTime;
    if (config.incremental && bfs::exists(config.headerFilepath))
    {
        ...
        maxLastWriteTime = existingHeaderLastWriteTime = bfs::last_write_time(config.headerFilepath);
        if (config.verbose)
            cout << config.headerFilepath << ": " << maxRevision << endl;
    }
    ...
    if (config.verbose)
        cout << "Maximum revision: " << maxRevision << endl;
    ...
    // the header must always be at least as recent as the most recent versioned filepath
    else if (existingHeaderLastWriteTime < maxLastWriteTime)
        bfs::last_write_time(config.headerFilepath, maxLastWriteTime);
