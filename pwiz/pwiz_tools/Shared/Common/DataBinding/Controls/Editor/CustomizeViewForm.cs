...
namespace pwiz.Common.DataBinding.Controls.Editor
{
    ...
    public partial class CustomizeViewForm : Form
    {
        ...
        {
            return ViewInfo.DisplayColumns.Where(IsCanonical).Select(dc => dc.PropertyPath);
        }
        ...
        private void RemoveColumn(PropertyPath propertyPath)
        {
        ...
        private void UpdatePropertySheet()
        {
            ...
            if (selectedColumns.Length == 1)
            {
                ...
                {
                    var columnDescriptor = ParentColumn.ResolveDescendant(selectedColumn.PropertyPath);
                    if (columnDescriptor != null)
                    {
        ...
        private int IndexOfCanconical(PropertyPath propertyPath)
        {
            ...
            {
                var displayColumn = ViewInfo.DisplayColumns[i];
                if (Equals(propertyPath, displayColumn.PropertyPath) && IsCanonical(displayColumn))
                {
                    return i;
                }
            }
        ...
        {
            var availableSublists = new HashSet<PropertyPath> {PropertyPath.Root};
            foreach (var columnSpec in ViewSpec.Columns)
            {
                for (PropertyPath idPath = columnSpec.PropertyPath; !idPath.IsRoot; idPath = idPath.Parent)
                {
                    if (idPath.Name == null)
                    {
        ...
        private string GetSublistLabel(PropertyPath propertyPath)
        {
            var parts = new List<string>();
            while (!propertyPath.IsRoot)
            {
                var treeNode = availableFieldsTreeColumns.FindTreeNode(propertyPath, true);
                if (treeNode == null)
                {
                    parts.Add(propertyPath.Name);
        ...
        class SublistItem
        {
            public SublistItem(string displayName, PropertyPath propertyPath)
            {
        ...
        private void ComboSublistOnSelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (sublistItem != null)
            {
                ViewSpec = _viewSpec.SetSublistId(sublistItem.PropertyPath);
            }
        }
        ...
        private void AvailableFieldsTreeColumnsOnNodeMouseDoubleClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            ...
            foreach (ListViewItem item in listViewColumns.Items)
            {
                ...
                if (Equals(columnDescriptor.PropertyPath, displayColumn.PropertyPath) && IsCanonical(displayColumn))
                {
                    item.Focused = true;
        ...
        {
            availableFieldsTreeColumns.SelectColumn(ViewSpec.Columns[listViewColumns.FocusedItem.Index].PropertyPath);
        }
