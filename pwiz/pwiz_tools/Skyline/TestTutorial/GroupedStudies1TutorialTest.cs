...
namespace pwiz.SkylineTestTutorial
{
    ...
    public class GroupedStudies1TutorialTest : AbstractFunctionalTestEx
    {
        ...
        private void ExploreTopPeptides()
        {
            ...
            SelectNode(SrmDocument.Level.Peptides, 2);
        ...
        private void PrepareForStatistics()
        {
...

            {
                ...
                AddReplicateAnnotation(annotationSettingsDlg, "SubjectId", AnnotationDef.AnnotationType.text, null, 46);

                RunUI(() => annotationSettingsDlg.AnnotationsCheckedListBox.SetItemChecked(0, true));

                PauseForScreenShot<ChooseAnnotationsDlg>("Annotation Settings form with SubjectId", 47);
...

            {
                ...
                AddReplicateAnnotation(annotationSettingsDlg, "BioReplicate", AnnotationDef.AnnotationType.text, null, 49);

                AddReplicateAnnotation(annotationSettingsDlg, "Condition", AnnotationDef.AnnotationType.value_list,
                    new[] {"Healthy", "Diseased"}, 49);

                RunUI(() =>
                {
                    annotationSettingsDlg.AnnotationsCheckedListBox.SetItemChecked(1, true);
                    annotationSettingsDlg.AnnotationsCheckedListBox.SetItemChecked(2, true);
                });

                PauseForScreenShot<ChooseAnnotationsDlg>("Annotation Settings form with MSstats annotations", 49);
...

            {
                ...
                AddAnnotation(annotationSettingsDlg, "MissingData", AnnotationDef.AnnotationType.true_false, null,
        ...
        private void AddReplicateAnnotation(ChooseAnnotationsDlg annotationSettingsDlg,
                                            ...
                                            int pausePage)
        {
            AddAnnotation(annotationSettingsDlg, annotationName, annotationType, annotationValues,                
                    AnnotationDef.AnnotationTargetSet.Singleton(AnnotationDef.AnnotationTarget.replicate),
                    pausePage);
        }

        private void AddAnnotation(ChooseAnnotationsDlg annotationSettingsDlg,
                                            string annotationName,
                                            AnnotationDef.AnnotationType annotationType,
                                            IList<string> annotationValues,
                                            ...
                                            int pausePage)
        {
            var annotationsListDlg = ShowDialog<EditListDlg<SettingsListBase<AnnotationDef>, AnnotationDef>>(annotationSettingsDlg.EditList);
