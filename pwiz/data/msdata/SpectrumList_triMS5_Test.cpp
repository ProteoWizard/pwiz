...
// adapted from Spectrum_List_mz5_Test.cpp
//
...
ostream* os_ = 0;
const char* testFilename = "SpectrumList_triMS5_Test.triMS5";
...
/**
* check for different preset scan configuration possibilities
*/
void testPreset()
{
	{
		...
		//Adopt the different preset scan configuration
		SpectrumListPtr spectrumList = tiny.run.spectrumListPtr;
		
		SpectrumPtr s19 = spectrumList->spectrum(0);
		s19->scanList.scans[0].set(MS_preset_scan_configuration, 1);

		SpectrumPtr s20 = spectrumList->spectrum(1);
		s20->scanList.scans[0].set(MS_preset_scan_configuration, 2);

		SpectrumPtr s21 = spectrumList->spectrum(2);
		s21->scanList.scans[0].set(MS_preset_scan_configuration,3);
		SpectrumPtr s22 = spectrumList->spectrum(3);
		s22->scanList.scans[0].set(MS_preset_scan_configuration, 3);

		SpectrumPtr s23 = spectrumList->spectrum(4);
		s23->scanList.scans[0].set(MS_preset_scan_configuration, 4);
		...
		// now check the easy functions again
		unit_assert(sl.get());
