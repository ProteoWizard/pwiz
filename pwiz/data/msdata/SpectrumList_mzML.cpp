...
void SpectrumList_mzMLImpl::readIndex()
{
    ...
    if (indexIndexOffset == string::npos)
        throw runtime_error("SpectrumList_mzML::readIndex()] <indexOffset> not found."); 

    is_->seekg(-bufferSize + static_cast<int>(indexIndexOffset), ios::end);
    if (!*is_)
        throw runtime_error("SpectrumList_mzML::readIndex()] Error seeking to <indexOffset>."); 
    ...
    if (indexOffset == 0)
        throw runtime_error("SpectrumList_mzML::readIndex()] Error parsing <indexOffset>."); 
