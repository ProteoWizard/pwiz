...
public static class VendorSdkLoader
{
    ...
    /// <summary>
    /// Pinned URLs name a commit in ProteoWizard/pwiz. When that repo's history is rewritten
    /// (the C++ tree was purged from it in 2026) a commit pinned by an older build stops
    /// resolving there, but the full pre-rewrite history is mirrored in ProteoWizard/pwiz-cpp,
    /// so the same path at the same commit is served from that mirror instead.
    /// </summary>
    private const string PrimaryRepoUrlSegment = "/ProteoWizard/pwiz/";
    private const string ArchiveRepoUrlSegment = "/ProteoWizard/pwiz-cpp/";

    private static IEnumerable<string> CandidateUrls(string url)
    {
        yield return url;
        if (url.Contains(PrimaryRepoUrlSegment, StringComparison.Ordinal))
            yield return url.Replace(PrimaryRepoUrlSegment, ArchiveRepoUrlSegment, StringComparison.Ordinal);
    }
    ...
    private static void DownloadIfMissing(VendorSdkPin entry, string dest)
    {
        ...
        try
        {
            foreach (string url in CandidateUrls(entry.Url))
            {
                Trace.TraceInformation($"[VendorSdkLoader] downloading {entry.Name} from {url}");
                response?.Dispose();
                response = http.GetAsync(url, HttpCompletionOption.ResponseHeadersRead).GetAwaiter().GetResult();
