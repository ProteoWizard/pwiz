...
End-to-end packaging pipeline:
  ...
  2. dotnet build pwiz_tools/MSConvertGUI/MsConvertGUI.csproj -c Release
  ...
  4. Download the .NET 10 desktop runtime installer EXE (cached under
     scripts/installer/cache/) so we can embed it in the Setup.exe
...
.USAGE
    ...
    pwsh -File scripts/installer/build.ps1 -SkipBuild   # dotnet output is fresh
#>
...
# Anchor every path off the repo root. Get-PwizSharpRoot uses sentinel
# discovery (pwiz/+pwiz_tools/+Pwiz.sln), so future tree restructures don't
# invalidate the paths below — only the discovery helper itself.
. (Join-Path $installerDir "../Get-PwizSharpRoot.ps1")
$pwizSharp       = $PwizSharpRoot
$msconvertGui    = Join-Path $pwizSharp "pwiz_tools/MSConvertGUI/MsConvertGUI.csproj"
$seems           = Join-Path $pwizSharp "pwiz_tools/SeeMS/SeeMS.csproj"
$msconvertGuiOut = Join-Path $pwizSharp "pwiz_tools/MSConvertGUI/bin/Release/net10.0-windows"
$seemsOut        = Join-Path $pwizSharp "pwiz_tools/SeeMS/bin/Release/net10.0-windows"
...
dotnet run --project $pinsGenProj -c Release -- $pwizSharp --require-all-pins
