...
REM #       Thermo) from vendor-archives\vendor_api_*.7z and link the
...
if %IAGREE%==1 (
    ...
    set BUILD_TARGET=pwiz_tools\Commandline\MsConvert\src\MsConvert.csproj
)
...
if %IAGREE%==1 (
    ...
    if !HAVE_ISCC!==1 (
        echo ##teamcity[progressMessage 'scripts/installer/build.ps1 -SkipBuild ^(uses Release artifacts^)']
        pwsh -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_DIR%\scripts\installer\build.ps1" -SkipBuild
        if !ERRORLEVEL! NEQ 0 (
...
set TEST_TARGET=
for %%r in ("%SCRIPT_DIR%\pwiz" "%SCRIPT_DIR%\scripts\installer" "%SCRIPT_DIR%\pwiz_tools\BiblioSpec" "%SCRIPT_DIR%\pwiz_tools\Commandline" "%SCRIPT_DIR%\pwiz_tools\MSConvertGUI") do (
