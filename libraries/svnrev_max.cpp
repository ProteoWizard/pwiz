...
Config parseCommandLine(int argc, const char* argv[])
{
    ...
    od_config.add_options()
        ...
        ("jamfile,j",
            po::value<string>(&config.jamFilepath)->default_value(""),
            ": if non-empty, the filepath of the jamfile to contain the revision info")
        ;
    ...
    if (config.filepaths.empty())
        throw runtime_error("[svnrev_max] No files specified.");
...
int go(const Config& config)
{
    ...
    {
        cout << "[svnrev_max] Updating revision info in " << config.headerFilepath << "." << endl;

        bfs::ofstream header(config.headerFilepath);
        imbueDateFacet(header);

        header << "// This file was generated by the \"svnrev_max\" utility\n"
        ...
        if (!config.jamFilepath.empty())
        {
            cout << "[svnrev_max] Updating revision info in " << config.jamFilepath << "." << endl;

            bfs::ofstream jamfile(config.jamFilepath);
            imbueDateFacet(jamfile);

            jamfile << "# This file was generated by the \"svnrev_max\" utility\n"
                    << "# You should not modify it manually, as it may be re-generated.\n"
                    << "#\n"
                    // $ is split from the rest of the keyword so that SVN doesn't replace it
                    << "# $" << "Revision: " << maxRevision.number << " $\n"
                    ...
                    << "#\n"
                    << endl
                    << "rule SVN_REV ( )     { return " << maxRevision.number << " ; }\n"
                    << "rule SVN_REVDATE ( ) { return " << maxRevision.date << " ; }\n"
                    << endl;
        }
    }
    ...
    {
        cout << "[svnrev_max] Updating last write time of " << config.headerFilepath << "." << endl;
        bfs::last_write_time(config.headerFilepath, maxLastWriteTime+1);
        if (!config.jamFilepath.empty())
        {
            cout << "[svnrev_max] Updating last write time of " << config.jamFilepath << "." << endl;
            bfs::last_write_time(config.jamFilepath, maxLastWriteTime+1);
        }
    }
    else if (config.verbose)
    {
        cout << "[svnrev_max] " << config.headerFilepath << " is up to date." << endl;
        if (!config.jamFilepath.empty())
        {
            cout << "[svnrev_max] " << config.jamFilepath << " is up to date." << endl;
        }
    }
